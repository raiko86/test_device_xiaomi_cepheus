...
Return<RequestStatus> BiometricsFingerprint::ErrorFilter(int32_t error) {
  switch (error) {
    ...
    default:
      ALOGE("An unknown error returned from fingerprint vendor library: %d",
            error);
...
Return<RequestStatus> BiometricsFingerprint::setActiveGroup(
  ...
  if (android::base::StartsWith(mutableStorePath, "/data/system/users/")) {
    ...
    mutableStorePath += static_cast<std::string>(storePath).substr(
        strlen("/data/system/users/"));
  }
  ...
  return ErrorFilter(
      mDevice->set_active_group(mDevice, gid, mutableStorePath.c_str()));
}
...
fingerprint_device_t* getDeviceForVendor(const char* class_name) {
  ...
  err = hw_get_module_by_class(FINGERPRINT_HARDWARE_MODULE_ID, class_name,
                               &hw_module);
  if (err) {
    ALOGE("Failed to get fingerprint module: class %s, error %d", class_name,
          err);
    return nullptr;
  }
  ...
  if (err) {
    ALOGE("Can't open fingerprint methods, class %s, error: %d", class_name,
          err);
    return nullptr;
  }

  if (kVersion != device->version) {
    ALOGE("Wrong fingerprint version: expected %d, got %d", kVersion,
          device->version);
    return nullptr;
  }
...
void BiometricsFingerprint::notify(const fingerprint_msg_t* msg) {
  ...
  switch (msg->type) {
    case FINGERPRINT_ERROR: {
      ...
      if (!thisPtr->mClientCallback->onError(devId, result, vendorCode)
               .isOk()) {
    ...
    case FINGERPRINT_AUTHENTICATED:
      if (msg->data.authenticated.finger.fid != 0) {
        ...
        const hidl_vec<uint8_t> token(std::vector<uint8_t>(
            hat, hat + sizeof(msg->data.authenticated.hat)));
        if (!thisPtr->mClientCallback
    ...
    case FINGERPRINT_TEMPLATE_ENUMERATING:
      ALOGD("onEnumerate(fid=%d, gid=%d, rem=%d)",
            msg->data.enumerated.finger.fid, msg->data.enumerated.finger.gid,
...
Return<void> BiometricsFingerprint::onFingerDown(uint32_t /* x */,
                                                 uint32_t /* y */,
                                                 float /* minor */,
                                                 float /* major */) {
