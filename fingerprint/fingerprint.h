 ...
 * that results in a situation where the current operation can't continue
 * without user interaction. For example, if the sensor is dirty during
 * enrollment and no further enrollment progress can be made, send
 * FINGERPRINT_ACQUIRED_IMAGER_DIRTY followed by FINGERPRINT_ERROR_CANCELED.
 */
 ...
 * Anything but FINGERPRINT_ACQUIRED_GOOD will be shown to the user as feedback
 * on how to take action on the current operation. For example,
 * FINGERPRINT_ACQUIRED_IMAGER_DIRTY can be used to tell the user to clean the
 * sensor.  If this will cause the current operation to fail, an additional
 * FINGERPRINT_ERROR_CANCELED can be sent to stop the operation in progress
 * (e.g. enrollment). In general, these messages will result in a "Try again"
 * message.
 */
typedef enum fingerprint_acquired_info {
  ...
  FINGERPRINT_ACQUIRED_DETECTED =
      6, /* when the finger is first detected. Used to optimize wakeup.
            Should be followed by one of the above messages */
  FINGERPRINT_ACQUIRED_VENDOR_BASE =
      1000 /* vendor-specific acquisition messages start here */
...
typedef struct fingerprint_device {
   ...
   *                  or a negative number in case of error, generally from the
   * errno.h set.
   */
   ...
   * Generates a unique token to upper layers to indicate the start of an
   * enrollment transaction. This token will be wrapped by security for
   * verification and passed to enroll() for verification before enrollment will
   * be allowed. This is to ensure adding a new fingerprint template was
   * preceded by some kind of credential confirmation (e.g. device password).
   *
   ...
   * The fingerprint template will be assigned to the group gid. User has a
   * choice to supply the gid or set it to 0 in which case a unique group id
   * will be generated.
   *
   * Function return: 0 if enrollment process can be successfully started
   *                  or a negative number in case of error, generally from the
   * errno.h set. A notify() function may be called indicating the error
   * condition.
   */
  ...
  /*
   * Finishes the enroll operation and invalidates the pre_enroll() generated
   * challenge. This will be called at the end of a multi-finger enrollment
   * session to indicate that no more fingers will be added.
   *
   ...
   * will change whenever a new fingerprint is enrolled, thus creating a new
   * fingerprint set.
   *
   ...
   * state. Unlike enroll_done() doesn't invalidate the pre_enroll() challenge.
   *
   ...
   * fingerprint_msg.data.enumerated.remaining_templates indicating how many
   * more enumeration messages to expect. Note: If there are no fingerprints,
   * then this should return 0 and the first fingerprint enumerated should have
   * fingerid=0 and remaining=0 Function return: 0 if enumerate request is
   * accepted or a negative number in case of error, generally from the errno.h
   * set.
   */
