...
BatchingAdapter::BatchingAdapter()
    : LocAdapterBase(0, LocContext::getLocContext(
...
void BatchingAdapter::readConfigCommand() {
  ...
  struct MsgReadConfig : public LocMsg {
    ...
    inline virtual void proc() const {
      ...
      LOC_LOGD(
          "%s]: batchSize %u tripBatchSize %u batchingAccuracy %u "
          "batchingTimeout %u ",
          __func__, batchSize, tripBatchSize, batchingAccuracy,
          batchingTimeout);
...
void BatchingAdapter::restartSessions() {
  ...
  for (auto it = mBatchingSessions.begin(); it != mBatchingSessions.end();
       ++it) {
    if (it->second.batchingMode != BATCHING_MODE_TRIP) {
      mLocApi->startBatching(
          it->first.id, it->second, getBatchingAccuracy(), getBatchingTimeout(),
...
bool BatchingAdapter::isBatchingSession(LocationAPI* client,
                                        uint32_t sessionId) {
...
void BatchingAdapter::saveBatchingSession(
...
void BatchingAdapter::eraseBatchingSession(LocationAPI* client,
                                           uint32_t sessionId) {
  LocationSessionKey key(client, sessionId);
...
void BatchingAdapter::reportResponse(LocationAPI* client, LocationError err,
                                     uint32_t sessionId) {
  ...
  if (it != mClientData.end() && it->second.responseCb != nullptr) {
    ...
    LOC_LOGE("%s]: client %p id %u not found in data", __func__, client,
             sessionId);
  }
}
...
uint32_t BatchingAdapter::startBatchingCommand(LocationAPI* client,
                                               BatchingOptions& batchOptions) {
  uint32_t sessionId = generateSessionId();
  LOC_LOGD(
      "%s]: client %p id %u minInterval %u minDistance %u mode %u Batching "
      "Mode %d",
      __func__, client, sessionId, batchOptions.minInterval,
      batchOptions.minDistance, batchOptions.mode, batchOptions.batchingMode);
  ...
  struct MsgStartBatching : public LocMsg {
    ...
    inline MsgStartBatching(BatchingAdapter& adapter, LocApiBase& api,
                            LocationAPI* client, uint32_t sessionId,
                            BatchingOptions batchOptions)
        : LocMsg(),
          mAdapter(adapter),
    ...
    inline virtual void proc() const {
      ...
      if (LOCATION_ERROR_SUCCESS == err) {
        if (mBatchingOptions.batchingMode == BATCHING_MODE_ROUTINE ||
          ...
          mAdapter.startTripBatchingMultiplex(mClient, mSessionId,
                                              mBatchingOptions);
        } else {
          mAdapter.reportResponse(mClient, LOCATION_ERROR_INVALID_PARAMETER,
                                  mSessionId);
        }
      }
    }
  ...
  sendMsg(
      new MsgStartBatching(*this, *mLocApi, client, sessionId, batchOptions));
...
void BatchingAdapter::startBatching(LocationAPI* client, uint32_t sessionId,
  ...
  if (batchingOptions.batchingMode != BATCHING_MODE_NO_AUTO_REPORT &&
    ...
    // if there is currenty no batching sessions interested in batch full event,
    // then this new session will need to register for batch full event
    updateEvtMask(LOC_API_ADAPTER_BIT_BATCH_FULL,
                  LOC_REGISTRATION_MASK_ENABLED);
  }
  ...
  mLocApi->startBatching(
      sessionId, batchingOptions, getBatchingAccuracy(), getBatchingTimeout(),
      new LocApiResponse(*getContext(), [this, client, sessionId,
        ...
        if (LOCATION_ERROR_SUCCESS != err &&
            ...
            0 == autoReportBatchingSessionsCount()) {
          // if we fail to start batching and we have already registered batch
          // full event we need to undo that since no sessions are now
          // interested in batch full event
          updateEvtMask(LOC_API_ADAPTER_BIT_BATCH_FULL,
...
void BatchingAdapter::updateBatchingOptionsCommand(
    LocationAPI* client, uint32_t id, BatchingOptions& batchOptions) {
  LOC_LOGD(
      "%s]: client %p id %u minInterval %u minDistance %u mode %u batchMode %u",
      __func__, client, id, batchOptions.minInterval, batchOptions.minDistance,
  ...
  struct MsgUpdateBatching : public LocMsg {
    ...
    inline MsgUpdateBatching(BatchingAdapter& adapter, LocApiBase& api,
                             LocationAPI* client, uint32_t sessionId,
                             BatchingOptions batchOptions)
        : LocMsg(),
          mAdapter(adapter),
    ...
    inline virtual void proc() const {
      ...
      if (LOCATION_ERROR_SUCCESS == err) {
        if (!mAdapter.isTripSession(mSessionId)) {
          ...
          mAdapter.stopTripBatchingMultiplex(mClient, mSessionId, true,
                                             mBatchOptions);
        }
      }
    }
...
void BatchingAdapter::stopBatchingCommand(LocationAPI* client, uint32_t id) {
  ...
  struct MsgStopBatching : public LocMsg {
    ...
    LocationAPI* mClient;
    uint32_t mSessionId;
    inline MsgStopBatching(BatchingAdapter& adapter, LocApiBase& api,
                           LocationAPI* client, uint32_t sessionId)
        : LocMsg(),
          mAdapter(adapter),
          mApi(api),
...
void BatchingAdapter::stopBatching(LocationAPI* client, uint32_t sessionId,
                                   bool restartNeeded,
    ...
    mLocApi->stopBatching(
        ...
        new LocApiResponse(*getContext(), [this, client, sessionId, flpOptions,
          ...
          if (LOCATION_ERROR_SUCCESS != err) {
            ...
            // if stopBatching is success, unregister for batch full event if
            // this was the last batching session that is interested in batch
            // full event
...
void BatchingAdapter::getBatchedLocationsCommand(LocationAPI* client,
...
void BatchingAdapter::reportLocationsEvent(const Location* locations,
                                           size_t count,
                                           BatchingMode batchingMode) {
  ...
  struct MsgReportLocations : public LocMsg {
    ...
    inline virtual ~MsgReportLocations() {
      if (nullptr != mLocations) delete[] mLocations;
    }
    inline virtual void proc() const {
...
void BatchingAdapter::reportCompletedTripsEvent(uint32_t accumulated_distance) {
  struct MsgReportCompletedTrips : public LocMsg {
    ...
    inline virtual void proc() const {
      ...
      for (auto itt = mAdapter.mTripSessions.begin();
           itt != mAdapter.mTripSessions.end();) {
        ...
        tripSession.accumulatedDistanceThisTrip =
            ...
            (mAccumulatedDistance -
             tripSession.accumulatedDistanceOngoingBatch);
        if (tripSession.tripDistance <=
            tripSession.accumulatedDistanceThisTrip) {
          // trip is completed
  ...
  LOC_LOGD("%s]: Accumulated Distance so far: %u", __func__,
           accumulated_distance);

  sendMsg(new MsgReportCompletedTrips(*this, accumulated_distance));
}

void BatchingAdapter::reportBatchStatusChange(
    BatchingStatus batchStatus, std::list<uint32_t>& completedTripsList) {
  BatchingStatusInfo batchStatusInfo = {sizeof(BatchingStatusInfo),
                                        batchStatus};
...
void BatchingAdapter::startTripBatchingMultiplex(
    LocationAPI* client, uint32_t sessionId,
    const BatchingOptions& batchingOptions) {
  if (mTripSessions.size() == 0) {
    // if there is currenty no batching sessions interested in batch full event,
    ...
    mLocApi->startOutdoorTripBatching(
        batchingOptions.minDistance, batchingOptions.minInterval,
        getBatchingTimeout(),
        new LocApiResponse(*getContext(), [this, client, sessionId,
                                           batchingOptions](LocationError err) {
    ...
    mLocApi->queryAccumulatedTripDistance(
        new LocApiResponseData<LocApiBatchData>(
            *getContext(), [this, batchingOptions, sessionId, client](
              ...
              TripSessionStatus newTripSession = {
                  accumulatedDistanceOngoingBatch, 0, 0,
                  batchingOptions.minDistance, batchingOptions.minInterval};
              if (err != LOCATION_ERROR_SUCCESS) {
                // unable to query accumulated distance, assume remaining
                // distance in ongoing batch is mongoingTripDistance.
                ...
                // check if new trip distance is lesser than the ongoing batch
                // remaining distance
                if (batchingOptions.minDistance <
                    ongoing_trip_remaining_distance) {
                  ongoingTripDistance = batchingOptions.minDistance;
                  needsRestart = true;
                } else if (needsRestart == true) {
                  // needsRestart is anyways true , may be because of lesser TBF
                  // of new session.
              ...
              if (needsRestart) {
                ...
                // reset the accumulatedDistanceOngoingBatch for each session,
                // and record the total accumulated distance so far for the
                // session.
                ...
                mLocApi->reStartOutdoorTripBatching(
                    ongoingTripDistance, ongoingTripInterval,
                    getBatchingTimeout(),
                    new LocApiResponse(*getContext(), [this, client, sessionId](
                                                          LocationError err) {
                      if (err != LOCATION_ERROR_SUCCESS) {
...
void BatchingAdapter::stopTripBatchingMultiplex(
    LocationAPI* client, uint32_t sessionId, bool restartNeeded,
    const BatchingOptions& batchOptions) {
  LocationError err = LOCATION_ERROR_SUCCESS;
  ...
  if (mTripSessions.size() == 1) {
    mLocApi->stopOutdoorTripBatching(
        true, new LocApiResponse(
                  *getContext(), [this, restartNeeded, client, sessionId,
                                  batchOptions](LocationError err) {
                    if (LOCATION_ERROR_SUCCESS == err) {
                      // if stopOutdoorTripBatching is success, unregister for
                      // batch full event if this was the last batching session
                      // that is interested in batch full event
...
void BatchingAdapter::stopTripBatchingMultiplexCommon(
    LocationError err, LocationAPI* client, uint32_t sessionId,
...
void BatchingAdapter::restartTripBatching(bool queryAccumulatedDistance,
                                          uint32_t accDist,
  ...
  mLocApi->queryAccumulatedTripDistance(new LocApiResponseData<LocApiBatchData>(
      *getContext(),
      [this, queryAccumulatedDistance, minRemainingDistance, minTBFInterval,
       accDist, numbatchedPos](LocationError /*err*/, LocApiBatchData data) {
        ...
        if (needsRestart) {
          mLocApi->reStartOutdoorTripBatching(
              ...
              new LocApiResponse(
                  *getContext(),
                  [this, accumulatedDistance, ongoingTripDistance,
                   ongoingTripInterval](LocationError err) {
                    if (err == LOCATION_ERROR_SUCCESS) {
                      for (auto itt = mTripSessions.begin();
                           itt != mTripSessions.end(); itt++) {
                        TripSessionStatus& tripSessStatus = itt->second;
