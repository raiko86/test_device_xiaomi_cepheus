...
using ::android::hardware::gnss::visibility_control::V1_0::implementation::
...
void Gnss::GnssDeathRecipient::serviceDied(uint64_t cookie,
                                           const wp<IBase>& who) {
...
Return<bool> Gnss::setCallback(const sp<V1_0::IGnssCallback>& callback) {
  ...
  // In case where previous call to
  // setCallback_1_1/setCallback_2_0/setCallback_2_1, then we need to cleanup
  // these interfaces/callbacks here since we no longer do so in cleanup()
  // function to keep callbacks around after cleanup()
  if (mApi != nullptr) {
...
Return<bool> Gnss::updateConfiguration(GnssConfig& gnssConfig) {
  ...
  if (api) {
    ...
    if (gnssConfig.flags & GNSS_CONFIG_FLAGS_SET_ASSISTANCE_DATA_VALID_BIT) {
      ...
      mPendingConfig.assistanceServer.size =
          sizeof(GnssConfigSetAssistanceServer);
    ...
    if (gnssConfig.flags &
      ...
      mPendingConfig.flags |=
          GNSS_CONFIG_FLAGS_AGLONASS_POSITION_PROTOCOL_VALID_BIT;
      mPendingConfig.aGlonassPositionProtocolMask =
          gnssConfig.aGlonassPositionProtocolMask;
    }
    if (gnssConfig.flags & GNSS_CONFIG_FLAGS_EM_PDN_FOR_EM_SUPL_VALID_BIT) {
      mPendingConfig.flags |= GNSS_CONFIG_FLAGS_EM_PDN_FOR_EM_SUPL_VALID_BIT;
      mPendingConfig.emergencyPdnForEmergencySupl =
          gnssConfig.emergencyPdnForEmergencySupl;
    }
    ...
    if (gnssConfig.flags & GNSS_CONFIG_FLAGS_EMERGENCY_EXTENSION_SECONDS_BIT) {
      ...
      mPendingConfig.emergencyExtensionSeconds =
          gnssConfig.emergencyExtensionSeconds;
    }
  }
  return true;
}
...
Return<bool> Gnss::injectLocation(double latitudeDegrees,
  ...
  if (nullptr != gnssInterface) {
    gnssInterface->injectLocation(latitudeDegrees, longitudeDegrees,
                                  accuracyMeters);
    return true;
  } else {
    return false;
...
Return<void> Gnss::deleteAidingData(
    V1_0::IGnss::GnssAidingData aidingDataFlags) {
  ENTRY_LOG_CALLFLOW();
  GnssAPIClient* api = getApi();
  if (api) {
...
Return<bool> Gnss::setPositionMode(
...
Return<bool> Gnss::setCallback_1_1(const sp<V1_1::IGnssCallback>& callback) {
  ...
  if (!r.isOk()) {
    LOC_LOGE("%s] Error from gnssNameCb description=%s", __func__,
             r.description().c_str());
  }

  // In case where previous call to setCallback/setCallback_2_0/setCallback_2_1,
  // then we need to cleanup these interfaces/callbacks here since we no longer
...
Return<bool> Gnss::setPositionMode_1_1(
  ...
  if (api) {
    GnssPowerMode powerMode =
        lowPowerMode ? GNSS_POWER_MODE_M4 : GNSS_POWER_MODE_M2;
    retVal = api->gnssSetPositionMode(mode, recurrence, minIntervalMs,
                                      preferredAccuracyMeters, preferredTimeMs,
                                      powerMode, minIntervalMs);
  }
  return retVal;
}
...
Return<sp<V1_1::IGnssMeasurement>> Gnss::getExtensionGnssMeasurement_1_1() {
  ...
  return nullptr;
#else
  if (mGnssMeasurement == nullptr) mGnssMeasurement = new GnssMeasurement();
  return mGnssMeasurement;
#endif
}
...
void Gnss::odcpiRequestCb(const OdcpiRequestInfo& request) {
  ...
  if (mGnssCbIface_2_1 != nullptr) {
    ...
    if (ODCPI_REQUEST_TYPE_START == request.type) {
      LOC_LOGd("gnssRequestLocationCb_2_1 isUserEmergency = %d",
               request.isEmergencyMode);
      auto r = mGnssCbIface_2_1->gnssRequestLocationCb_2_0(
          !request.isEmergencyMode, request.isEmergencyMode);
      if (!r.isOk()) {
        LOC_LOGe("Error invoking gnssRequestLocationCb_2_0 %s",
                 r.description().c_str());
      }
    } else {
    ...
    // Mark Independent from GNSS flag to false.
    if (ODCPI_REQUEST_TYPE_START == request.type) {
      LOC_LOGd("gnssRequestLocationCb_2_0 isUserEmergency = %d",
               request.isEmergencyMode);
      auto r = mGnssCbIface_2_0->gnssRequestLocationCb_2_0(
          !request.isEmergencyMode, request.isEmergencyMode);
      if (!r.isOk()) {
        LOC_LOGe("Error invoking gnssRequestLocationCb_2_0 %s",
                 r.description().c_str());
    ...
    if (ODCPI_REQUEST_TYPE_START == request.type) {
      auto r =
          mGnssCbIface_1_1->gnssRequestLocationCb(!request.isEmergencyMode);
      if (!r.isOk()) {
        LOC_LOGe("Error invoking gnssRequestLocationCb %s",
                 r.description().c_str());
      }
    } else {
      LOC_LOGv("Unsupported ODCPI request type: %d", request.type);
...
Return<bool> Gnss::setCallback_2_0(const sp<V2_0::IGnssCallback>& callback) {
  ...
  // In case where previous call to setCallback/setCallback_1_1/setCallback_2_1,
  // then we need to cleanup these interfaces/callbacks here since we no longer
  // do so in cleanup() function to keep callbacks around after cleanup()
  if (mApi != nullptr) {
...
Return<sp<::android::hardware::gnss::visibility_control::V1_0::
...
Return<bool> Gnss::injectBestLocation_2_0(
    const V2_0::GnssLocation& gnssLocation) {
  ENTRY_LOG_CALLFLOW();
  const GnssInterface* gnssInterface = getGnssInterface();
  if (nullptr != gnssInterface) {
    Location location = {};
...
Return<bool> Gnss::setCallback_2_1(const sp<V2_1::IGnssCallback>& callback) {
  ...
  // In case where previous call to setCallback/setCallback_1_1/setCallback_2_0,
  // then we need to cleanup these interfaces/callbacks here since we no longer
  // do so in cleanup() function to keep callbacks around after cleanup()
  if (mApi != nullptr) {
    mApi->gnssUpdateCallbacks(nullptr, nullptr);
