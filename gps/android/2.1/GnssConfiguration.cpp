...
Return<bool> GnssConfiguration::setSuplMode(uint8_t mode) {
  ...
  switch (mode) {
    ...
    case 3:
      config.suplModeMask =
          GNSS_CONFIG_SUPL_MODE_MSB_BIT | GNSS_CONFIG_SUPL_MODE_MSA_BIT;
      break;
    default:
...
Return<bool> GnssConfiguration::setBlacklist(
  ...
  for (int idx = 0; idx < (int)blacklist.size(); idx++) {
    ...
    blValid = setBlacklistedSource(
                  source, (GnssConstellationType)blacklist[idx].constellation,
                  blacklist[idx].svid) ||
              blValid;
...
Return<bool> GnssConfiguration::setEsExtensionSec(
