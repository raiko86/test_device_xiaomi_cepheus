...
#define SINGLE_SHOT_MIN_TRACKING_INTERVAL_MSEC \
  (590 * 60 * 60 * 1000)  // 590 hours
...
void GnssAPIClient::setCallbacks() {
  ...
  if (mGnssNiCbIface != nullptr) {
    loc_core::ContextBase* context = loc_core::LocContext::getLocContext(
        NULL, NULL, loc_core::LocContext::mLocationHalName, false);
    if (!context->hasAgpsExtendedCapabilities()) {
      LOC_LOGD("Registering NI CB");
      locationCallbacks.gnssNiCb = [this](uint32_t id,
                                          GnssNiNotification gnssNiNotify) {
  ...
  locationCallbacks.gnssNmeaCb =
      [this](GnssNmeaNotification gnssNmeaNotification) {
...
void GnssAPIClient::gnssUpdateCallbacks_2_0(
    const sp<V2_0::IGnssCallback>& gpsCb) {
  LOC_LOGD("%s]: (%p)", __FUNCTION__, &gpsCb);
...
void GnssAPIClient::gnssUpdateCallbacks_2_1(
    const sp<V2_1::IGnssCallback>& gpsCb) {
  LOC_LOGD("%s]: (%p)", __FUNCTION__, &gpsCb);
...
bool GnssAPIClient::gnssSetPositionMode(
  ...
  LOC_LOGD("%s]: (%d %d %d %d %d %d %d)", __FUNCTION__, (int)mode, recurrence,
  ...
  if (IGnss::GnssPositionMode::MS_ASSISTED == mode ||
    ...
    // We set a very large interval to simulate SINGLE mode. Once we report a
    // fix, the caller should take the responsibility to stop the session. For
    // MSA, we always treat it as SINGLE mode.
...
// for GpsNiInterface
void GnssAPIClient::gnssNiRespond(
    int32_t notifId, IGnssNiCallback::GnssUserResponseType userResponse) {
  LOC_LOGD("%s]: (%d %d)", __FUNCTION__, notifId,
           static_cast<int>(userResponse));
  GnssNiResponse data;
  switch (userResponse) {
...
void GnssAPIClient::gnssDeleteAidingData(
    IGnss::GnssAidingData aidingDataFlags) {
  ...
  data.sv.svTypeMask =
      GNSS_AIDING_DATA_SV_TYPE_GPS_BIT | GNSS_AIDING_DATA_SV_TYPE_GLONASS_BIT |
...
void GnssAPIClient::requestCapabilities() {
  // only send capablities if it's already cached, otherwise the first time
  // LocationAPI is initialized, capabilities will be sent by LocationAPI
...
// callbacks
void GnssAPIClient::onCapabilitiesCb(
  ...
  if (gnssCbIface_2_1 != nullptr || gnssCbIface_2_0 != nullptr ||
      gnssCbIface != nullptr) {
    ...
    if (capabilitiesMask & LOCATION_CAPABILITIES_GNSS_MEASUREMENTS_BIT) {
      ...
      if (capabilitiesMask & LOCATION_CAPABILITIES_DEBUG_NMEA_BIT) {
        ...
        if (capabilitiesMask &
                LOCATION_CAPABILITIES_CONSTELLATION_ENABLEMENT_BIT ||
          ...
          if (capabilitiesMask & LOCATION_CAPABILITIES_PRIVACY_BIT) {
            ...
            if (capabilitiesMask &
                LOCATION_CAPABILITIES_MEASUREMENTS_CORRECTION_BIT) {
    ...
    LOC_LOGV("%s:%d] set_system_info_cb (%d)", __FUNCTION__, __LINE__,
             gnssInfo.yearOfHw);
...
void GnssAPIClient::onTrackingCb(Location location) {
  ...
  LOC_LOGD("%s]: (flags: %02x isTracking: %d)", __FUNCTION__, location.flags,
           isTracking);
...
void GnssAPIClient::onGnssNiCb(uint32_t id,
  ...
  if (gnssNiNotification.timeoutResponse == GNSS_NI_RESPONSE_ACCEPT)
    ...
    notificationGnss.defaultResponse =
        IGnssNiCallback::GnssUserResponseType::RESPONSE_NORESP;
  ...
  else if (gnssNiNotification.requestorEncoding ==
  ...
  if (gnssNiNotification.messageEncoding == GNSS_NI_ENCODING_TYPE_NONE)
    notificationGnss.notificationIdEncoding =
        IGnssNiCallback::GnssNiEncodingType::ENC_NONE;
  else if (gnssNiNotification.messageEncoding ==
           GNSS_NI_ENCODING_TYPE_GSM_DEFAULT)
    notificationGnss.notificationIdEncoding =
        IGnssNiCallback::GnssNiEncodingType::ENC_SUPL_GSM_DEFAULT;
...
void GnssAPIClient::onGnssNmeaCb(GnssNmeaNotification gnssNmeaNotification) {
  ...
  if (gnssCbIface != nullptr ||
      gnssCbIface_2_0 != nullptr | gnssCbIface_2_1 != nullptr) {
    ...
    while (std::getline(ss, each, '\n')) {
      ...
      if (gnssCbIface_2_1 != nullptr) {
        auto r = gnssCbIface_2_1->gnssNmeaCb(
            static_cast<V1_0::GnssUtcTime>(gnssNmeaNotification.timestamp),
            nmeaString);
        if (!r.isOk()) {
          LOC_LOGE(
              "%s] Error from gnssCbIface_2_1 nmea=%s length=%u description=%s",
        ...
        if (!r.isOk()) {
          LOC_LOGE(
              "%s] Error from gnssCbIface_2_0 nmea=%s length=%u description=%s",
              __func__, gnssNmeaNotification.nmea, gnssNmeaNotification.length,
              r.description().c_str());
        }
      } else if (gnssCbIface != nullptr) {
        ...
        if (!r.isOk()) {
          LOC_LOGE("%s] Error from gnssNmeaCb nmea=%s length=%u description=%s",
                   __func__, gnssNmeaNotification.nmea,
                   gnssNmeaNotification.length, r.description().c_str());
        }
      }
    }
  }
...
void GnssAPIClient::onStartTrackingCb(LocationError error) {
  ...
  if (error == LOCATION_ERROR_SUCCESS) {
    if (gnssCbIface_2_1 != nullptr) {
      ...
      auto r = gnssCbIface_2_0->gnssStatusCb(
          IGnssCallback::GnssStatusValue::ENGINE_ON);
      if (!r.isOk()) {
        LOC_LOGE("%s] Error from gnssStatusCb 2_0 ENGINE_ON description=%s",
                 __func__, r.description().c_str());
      }
      r = gnssCbIface_2_0->gnssStatusCb(
          IGnssCallback::GnssStatusValue::SESSION_BEGIN);
      if (!r.isOk()) {
        LOC_LOGE("%s] Error from gnssStatusCb 2_0 SESSION_BEGIN description=%s",
                 __func__, r.description().c_str());
      ...
      auto r =
          gnssCbIface->gnssStatusCb(IGnssCallback::GnssStatusValue::ENGINE_ON);
      if (!r.isOk()) {
        LOC_LOGE("%s] Error from gnssStatusCb ENGINE_ON description=%s",
                 __func__, r.description().c_str());
      }
      r = gnssCbIface->gnssStatusCb(
          IGnssCallback::GnssStatusValue::SESSION_BEGIN);
      if (!r.isOk()) {
...
void GnssAPIClient::onStopTrackingCb(LocationError error) {
  ...
  if (error == LOCATION_ERROR_SUCCESS) {
    if (gnssCbIface_2_1 != nullptr) {
      ...
      auto r = gnssCbIface->gnssStatusCb(
          IGnssCallback::GnssStatusValue::SESSION_END);
      if (!r.isOk()) {
...
static void convertGnssSvStatus(GnssSvNotification& in,
  ...
  if (out.numSvs > static_cast<uint32_t>(V1_0::GnssMax::SVS_COUNT)) {
    LOC_LOGW("%s]: Too many satellites %u. Clamps to %d.", __FUNCTION__,
             out.numSvs, V1_0::GnssMax::SVS_COUNT);
    out.numSvs = static_cast<uint32_t>(V1_0::GnssMax::SVS_COUNT);
  }
  for (size_t i = 0; i < out.numSvs; i++) {
    convertGnssSvid(in.gnssSvs[i], out.gnssSvList[i].svid);
    convertGnssConstellationType(in.gnssSvs[i].type,
                                 out.gnssSvList[i].constellation);
    ...
    out.gnssSvList[i].svFlag =
        static_cast<uint8_t>(IGnssCallback::GnssSvFlags::NONE);
    if (in.gnssSvs[i].gnssSvOptionsMask & GNSS_SV_OPTIONS_HAS_EPHEMER_BIT)
      out.gnssSvList[i].svFlag |=
          IGnssCallback::GnssSvFlags::HAS_EPHEMERIS_DATA;
    ...
    if (in.gnssSvs[i].gnssSvOptionsMask &
...
static void convertGnssSvStatus(
    GnssSvNotification& in, hidl_vec<V2_0::IGnssCallback::GnssSvInfo>& out) {
  out.resize(in.count);
...
static void convertGnssSvStatus(
    GnssSvNotification& in, hidl_vec<V2_1::IGnssCallback::GnssSvInfo>& out) {
  out.resize(in.count);
  for (size_t i = 0; i < in.count; i++) {
