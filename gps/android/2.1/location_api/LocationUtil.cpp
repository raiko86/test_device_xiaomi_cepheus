...
bool getCurrentTime(struct timespec& currentTime, int64_t& sinceBootTimeNanos) {
  ...
  /* Attempt to get CLOCK_REALTIME and CLOCK_BOOTIME in succession without an
  interruption or context switch (for up to MAX_GET_TIME_COUNT times) to avoid
  ...
  for (uint32_t i = 0; i < MAX_GET_TIME_COUNT; i++) {
    ...
    sinceBootTimeNanos =
        sinceBootTime.tv_sec * 1000000000 + sinceBootTime.tv_nsec;
    int64_t sinceBootTimeTestNanos =
        sinceBootTimeTest.tv_sec * 1000000000 + sinceBootTimeTest.tv_nsec;
    int64_t sinceBootTimeDeltaNanos =
        sinceBootTimeTestNanos - sinceBootTimeNanos;

    /* sinceBootTime and sinceBootTimeTest should have a close value if there
    was no interruption or context switch between clock_gettime for
    CLOCK_BOOTIME and clock_gettime for CLOCK_REALTIME */
  ...
  if (getCurrentTime(currentTime, sinceBootTimeNanos)) {
    if (in.flags & LOCATION_HAS_ELAPSED_REAL_TIME) {
      ...
      LOC_LOGv("sinceBootTimeNanos:%" PRIi64 " in.elapsedRealTime=%" PRIi64
               ...
               sinceBootTimeNanos, in.elapsedRealTime, qTimerTickCount,
               qtimerDiff);
      ...
      if (currentTimeNanos >= locationTimeNanos) {
        ...
        if (ageTimeNanos <= 100000000) {
          ...
          out.elapsedRealtime.flags |=
              ElapsedRealtimeFlags::HAS_TIME_UNCERTAINTY_NS;
          // time uncertainty is the max value between abs(AP_UTC - MP_UTC) and
          // 100ms, to verify if user change the sys time
...
void convertSingleSatCorrections(const SingleSatCorrection& in,
  ...
  if (in.singleSatCorrectionFlags &
      (GnssSingleSatCorrectionFlags::HAS_SAT_IS_LOS_PROBABILITY)) {
...
void convertMeasurementCorrections(const MeasurementCorrectionsV1_0& in,
  ...
  out.horizontalPositionUncertaintyMeters =
      in.horizontalPositionUncertaintyMeters;
