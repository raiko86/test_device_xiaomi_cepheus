...
AGnss::AGnss(Gnss* gnss) : mGnss(gnss) { spAGnss = this; }

AGnss::~AGnss() { spAGnss = nullptr; }
...
Return<bool> AGnss::dataConnOpen(uint64_t /*networkHandle*/,
  ...
  mGnss->getGnssInterface()->agpsDataConnOpen(LOC_AGPS_TYPE_SUPL, apn.c_str(),
                                              apn.size(), (int)bearerType);
  return true;
}
...
Return<bool> AGnss::setServer(V2_0::IAGnssCallback::AGnssType type,
  ...
  if (type == IAGnssCallback::AGnssType::SUPL) {
    ...
    LOC_LOGE("%s]: invalid AGnssType: %d", __FUNCTION__,
