...
Return<void> GnssGeofencing::setCallback(
    const sp<IGnssGeofenceCallback>& callback) {
  if (mApi != nullptr) {
  ...
  if (mGnssGeofencingCbIface != nullptr) {
    mGnssGeofencingCbIface->linkToDeath(mGnssGeofencingDeathRecipient,
                                        0 /*cookie*/);
  }
...
Return<void> GnssGeofencing::addGeofence(
    int32_t geofenceId, double latitudeDegrees, double longitudeDegrees,
    double radiusMeters,
  ...
  if (mApi == nullptr) {
    ...
    mApi->geofenceAdd(geofenceId, latitudeDegrees, longitudeDegrees,
                      radiusMeters, static_cast<int32_t>(lastTransition),
...
Return<void> GnssGeofencing::resumeGeofence(int32_t geofenceId,
