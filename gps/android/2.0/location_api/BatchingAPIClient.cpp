...
static void convertBatchOption(const IGnssBatching::Options& in,
                               LocationOptions& out,
                               LocationCapabilitiesMask mask);

BatchingAPIClient::BatchingAPIClient(
    const sp<V1_0::IGnssBatchingCallback>& callback)
...
BatchingAPIClient::BatchingAPIClient(
    const sp<V2_0::IGnssBatchingCallback>& callback)
    : LocationAPIClientBase(),
      mGnssBatchingCbIface(nullptr),
      mDefaultId(UINT_MAX),
      mLocationCapabilitiesMask(0),
...
BatchingAPIClient::~BatchingAPIClient() { LOC_LOGD("%s]: ()", __FUNCTION__); }
...
void BatchingAPIClient::gnssUpdateCallbacks(
    const sp<V1_0::IGnssBatchingCallback>& callback) {
  mMutex.lock();
...
void BatchingAPIClient::gnssUpdateCallbacks_2_0(
    const sp<V2_0::IGnssBatchingCallback>& callback) {
  mMutex.lock();
...
int BatchingAPIClient::startSession(const IGnssBatching::Options& opts) {
  LOC_LOGD("%s]: (%lld %d)", __FUNCTION__,
           static_cast<long long>(opts.periodNanos),
  ...
  uint32_t mode = 0;
  if (opts.flags ==
      static_cast<uint8_t>(IGnssBatching::Flag::WAKEUP_ON_FIFO_FULL)) {
...
int BatchingAPIClient::updateSessionOptions(
    const IGnssBatching::Options& opts) {
  LOC_LOGD("%s]: (%lld %d)", __FUNCTION__,
           static_cast<long long>(opts.periodNanos),
           static_cast<uint8_t>(opts.flags));
  int retVal = -1;
  ...
  if (locAPIUpdateSessionOptions(mDefaultId, mode, options) ==
...
void BatchingAPIClient::onCapabilitiesCb(
...
static void convertBatchOption(const IGnssBatching::Options& in,
  ...
  if (mask & LOCATION_CAPABILITIES_GNSS_MSA_BIT) out.mode = GNSS_SUPL_MODE_MSA;
  if (mask & LOCATION_CAPABILITIES_GNSS_MSB_BIT) out.mode = GNSS_SUPL_MODE_MSB;
}
