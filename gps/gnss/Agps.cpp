...
void AgpsStateMachine::processAgpsEvent(AgpsEvent event) {
  LOC_LOGD("processAgpsEvent(): SM %p, Event %d, State %d", this, event,
           mState);
...
void AgpsStateMachine::processAgpsEventUnsubscribe() {
  switch (mState) {
    case AGPS_STATE_RELEASED:
      notifyEventToSubscriber(AGPS_EVENT_UNSUBSCRIBE, mCurrentSubscriber,
                              false);
      break;
...
void AgpsStateMachine::requestOrReleaseDataConn(bool request) {
  ...
  if (request) {
    LOC_LOGD("AGPS Data Conn Request mAgpsType=%d mApnTypeMask=0x%X", mAgpsType,
             mApnTypeMask);
    nifRequest.status = LOC_GPS_REQUEST_AGPS_DATA_CONN;
  } else {
    LOC_LOGD("AGPS Data Conn Release mAgpsType=%d mApnTypeMask=0x%X", mAgpsType,
             mApnTypeMask);
...
void AgpsStateMachine::notifyEventToSubscriber(
  ...
  switch (event) {
    case AGPS_EVENT_GRANTED:
      mAgpsManager->mAtlOpenStatusCb(subscriberToNotify->mConnHandle, 1,
                                     getAPN(), getAPNLen(), getBearer(),
                                     mAgpsType, mApnTypeMask);
      break;

    case AGPS_EVENT_DENIED:
      mAgpsManager->mAtlOpenStatusCb(subscriberToNotify->mConnHandle, 0,
                                     getAPN(), getAPNLen(), getBearer(),
                                     mAgpsType, mApnTypeMask);
      break;
...
void AgpsStateMachine::deleteSubscriber(AgpsSubscriber* subscriberToDelete) {
  LOC_LOGD("deleteSubscriber(): SM %p, Subscriber %p", this,
           subscriberToDelete);
...
void AgpsManager::createAgpsStateMachines(const AgpsCbInfo& cbInfo) {
  ...
  bool agpsCapable = ((loc_core::ContextBase::mGps_conf.CAPABILITIES &
                       LOC_GPS_CAPABILITY_MSA) ||
                      (loc_core::ContextBase::mGps_conf.CAPABILITIES &
                       LOC_GPS_CAPABILITY_MSB));

  if (NULL == mInternetNif && (cbInfo.atlType & AGPS_ATL_TYPE_WWAN)) {
    mInternetNif = new AgpsStateMachine(this, LOC_AGPS_TYPE_WWAN_ANY);
    mInternetNif->registerFrameworkStatusCallback(
...
void AgpsManager::requestATL(int connHandle, AGpsExtType agpsType,
  ...
  LOC_LOGD(
      "AgpsManager::requestATL(): connHandle %d, agpsType 0x%X apnTypeMask: "
      "0x%X",
      connHandle, agpsType, apnTypeMask);

  if (0 == loc_core::ContextBase::mGps_conf
  ...
  if (sm == NULL) {
    LOC_LOGE("No AGPS State Machine for agpsType: %d apnTypeMask: 0x%X",
             agpsType, apnTypeMask);
    mAtlOpenStatusCb(connHandle, 0, NULL, 0, AGPS_APN_BEARER_INVALID, agpsType,
                     apnTypeMask);
    return;
  }
  sm->setType(agpsType);
...
void AgpsManager::reportAtlOpenSuccess(AGpsExtType agpsType, char* apnName,
