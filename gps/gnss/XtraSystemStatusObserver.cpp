...
bool XtraSystemStatusObserver::updateConnections(
    uint64_t allConnections, NetworkInfoType* networkHandleInfo) {
...
inline bool XtraSystemStatusObserver::onStatusRequested(
    int32_t xtraStatusUpdated) {
...
void XtraSystemStatusObserver::startDgnssSource(
    const StartDgnssNtripParams& params) {
  stringstream ss;
...
void XtraSystemStatusObserver::notify(const list<IDataItemCore*>& dlist) {
  struct HandleOsObserverUpdateMsg : public LocMsg {
    ...
    inline ~HandleOsObserverUpdateMsg() {
      for (auto itor = mDataItemList.begin(); itor != mDataItemList.end();
           ++itor) {
        if (*itor != nullptr) {
          delete *itor;
          *itor = nullptr;
