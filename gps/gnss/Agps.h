...
typedef std::function<void(int handle, int isSuccess, char* apn,
...
class AgpsSubscriber {
 public:
  ...
  inline AgpsSubscriber(int connHandle, bool waitForCloseComplete,
                        bool isInactive, LocApnTypeMask apnTypeMask)
  ...
  inline virtual AgpsSubscriber* clone() {
    return new AgpsSubscriber(mConnHandle, mWaitForCloseComplete, mIsInactive,
                              mApnTypeMask);
  }
};
...
class AgpsStateMachine {
 ...
 public:
  ...
  inline void registerFrameworkStatusCallback(
 ...
 private:
  ...
  virtual void notifyEventToSubscriber(AgpsEvent event,
                                       AgpsSubscriber* subscriber,
...
class AgpsManager {
 ...
 public:
  ...
  inline bool isRegistered() {
    return nullptr != mAgnssNif || nullptr != mInternetNif;
  }
