 ...
 public:
  SystemStatusPQWP7parser(const char* str_in, uint32_t len_in)
    ...
    for (uint32_t i = 0; i < svLimit; i++) {
      ...
      mP7.mNav[i].mSource =
          GnssEphemerisSource(atoi(mField[i * 3 + 3].c_str()));
...
void SystemStatusTimeAndClock::dump() {
  LOC_LOGV(
      "TimeAndClock: u=%ld:%ld g=%d:%d v=%d ts=%d tu=%d b=%d bu=%d ls=%d lu=%d "
      "un=%" PRIu64,
      mUtcTime.tv_sec, mUtcTime.tv_nsec, mGpsWeek, mGpsTowMs, mTimeValid,
      mTimeSource, mTimeUnc, mClockFreqBias, mClockFreqBiasUnc, mLeapSeconds,
      mLeapSecUnc, mTimeUncNs);
  return;
}
...
void SystemStatusXoState::dump() {
  LOC_LOGV("XoState: u=%ld:%ld x=%d", mUtcTime.tv_sec, mUtcTime.tv_nsec,
           mXoState);
  return;
}
...
bool SystemStatusRfAndParams::equals(const SystemStatusRfAndParams& peer) {
  if ((mPgaGain != peer.mPgaGain) || (mGpsBpAmpI != peer.mGpsBpAmpI) ||
      (mGpsBpAmpQ != peer.mGpsBpAmpQ) || (mAdcI != peer.mAdcI) ||
      (mAdcQ != peer.mAdcQ) || (mJammerGps != peer.mJammerGps) ||
      (mJammerGlo != peer.mJammerGlo) || (mJammerBds != peer.mJammerBds) ||
      (mJammerGal != peer.mJammerGal) || (mAgcGps != peer.mAgcGps) ||
      (mAgcGlo != peer.mAgcGlo) || (mAgcBds != peer.mAgcBds) ||
...
void SystemStatusRfAndParams::dump() {
  LOC_LOGV(
      ...
      mUtcTime.tv_sec, mUtcTime.tv_nsec, mPgaGain, mGpsBpAmpI, mGpsBpAmpQ,
      mAdcI, mAdcQ, mJammerGps, mJammerGlo, mJammerBds, mJammerGal, mAgcGps,
      mAgcGlo, mAgcBds, mAgcGal);
  return;
}
...
void SystemStatusErrRecovery::dump() {
  LOC_LOGV("ErrRecovery: u=%ld:%ld e=%d", mUtcTime.tv_sec, mUtcTime.tv_nsec,
           mRecErrorRecovery);
  return;
}
...
bool SystemStatusInjectedPosition::equals(
    const SystemStatusInjectedPosition& peer) {
  if ((mEpiValidity != peer.mEpiValidity) || (mEpiLat != peer.mEpiLat) ||
      (mEpiLon != peer.mEpiLon) || (mEpiAlt != peer.mEpiAlt) ||
      (mEpiHepe != peer.mEpiHepe) || (mEpiAltUnc != peer.mEpiAltUnc) ||
...
void SystemStatusInjectedPosition::dump() {
  LOC_LOGV(
      "InjectedPosition: u=%ld:%ld v=%x la=%f lo=%f al=%f he=%f au=%f es=%d",
      mUtcTime.tv_sec, mUtcTime.tv_nsec, mEpiValidity, mEpiLat, mEpiLon,
      mEpiAlt, mEpiHepe, mEpiAltUnc, mEpiSrc);
  return;
}
...
bool SystemStatusBestPosition::equals(const SystemStatusBestPosition& peer) {
  if ((mBestLat != peer.mBestLat) || (mBestLon != peer.mBestLon) ||
      (mBestAlt != peer.mBestAlt) || (mBestHepe != peer.mBestHepe) ||
...
void SystemStatusBestPosition::dump() {
  LOC_LOGV("BestPosition: u=%ld:%ld la=%f lo=%f al=%f he=%f au=%f",
           mUtcTime.tv_sec, mUtcTime.tv_nsec, mBestLat, mBestLon, mBestAlt,
           mBestHepe, mBestAltUnc);
  return;
}
...
bool SystemStatusXtra::equals(const SystemStatusXtra& peer) {
  if ((mXtraValidMask != peer.mXtraValidMask) ||
      (mGpsXtraAge != peer.mGpsXtraAge) || (mGloXtraAge != peer.mGloXtraAge) ||
      (mBdsXtraAge != peer.mBdsXtraAge) || (mGalXtraAge != peer.mGalXtraAge) ||
...
void SystemStatusXtra::dump() {
  LOC_LOGV("SystemStatusXtra: u=%ld:%ld m=%x a=%d:%d:%d:%d:%d v=%x:%x:%" PRIx64
           ":%" PRIx64 ":%x",
           mUtcTime.tv_sec, mUtcTime.tv_nsec, mXtraValidMask, mGpsXtraAge,
           mGloXtraAge, mBdsXtraAge, mGalXtraAge, mQzssXtraAge, mGpsXtraValid,
           mGloXtraValid, mBdsXtraValid, mGalXtraValid, mQzssXtraValid);
  return;
}
...
void SystemStatusEphemeris::dump() {
  LOC_LOGV("Ephemeris: u=%ld:%ld ev=%x:%x:%" PRIx64 ":%" PRIx64 ":%x",
           mUtcTime.tv_sec, mUtcTime.tv_nsec, mGpsEpheValid, mGloEpheValid,
           mBdsEpheValid, mGalEpheValid, mQzssEpheValid);
  return;
}
...
void SystemStatusSvHealth::dump() {
  LOC_LOGV(
      ...
      mUtcTime.tv_sec, mUtcTime.tv_nsec, mGpsUnknownMask, mGloUnknownMask,
      mBdsUnknownMask, mGalUnknownMask, mQzssUnknownMask, mGpsGoodMask,
      mGloGoodMask, mBdsGoodMask, mGalGoodMask, mQzssGoodMask, mGpsBadMask,
      mGloBadMask, mBdsBadMask, mGalBadMask, mQzssBadMask);
  return;
}
...
void SystemStatusPdr::dump() {
  LOC_LOGV("Pdr: u=%ld:%ld m=%x", mUtcTime.tv_sec, mUtcTime.tv_nsec,
           mFixInfoMask);
  return;
}
...
void SystemStatusNavData::dump() {
  ...
  for (uint32_t i = 0; i < SV_ALL_NUM; i++) {
    LOC_LOGV("i=%d type=%d src=%d age=%d", i, mNav[i].mType, mNav[i].mSource,
             mNav[i].mAgeSec);
  }
  return;
}
...
SystemStatusPositionFailure::SystemStatusPositionFailure(
    const SystemStatusPQWS1& nmea)
    : mFixInfoMask(nmea.mFixInfoMask), mHepeLimit(nmea.mHepeLimit) {}

bool SystemStatusPositionFailure::equals(
    const SystemStatusPositionFailure& peer) {
...
void SystemStatusPositionFailure::dump() {
  LOC_LOGV("PositionFailure: u=%ld:%ld m=%d h=%d", mUtcTime.tv_sec,
           mUtcTime.tv_nsec, mFixInfoMask, mHepeLimit);
  return;
}
...
bool SystemStatusLocation::equals(const SystemStatusLocation& peer) {
  if ((mLocation.gpsLocation.latitude != peer.mLocation.gpsLocation.latitude) ||
      (mLocation.gpsLocation.longitude !=
       peer.mLocation.gpsLocation.longitude) ||
...
IOsObserver* SystemStatus::getOsObserver() { return &mSysStatusObsvr; }
...
template <typename TYPE_REPORT, typename TYPE_ITEM>
void SystemStatus::setDefaultIteminReport(TYPE_REPORT& report,
                                          const TYPE_ITEM& s) {
  report.push_back(s);
  if (report.size() > s.maxItem) {
    report.erase(report.begin());
...
template <typename TYPE_REPORT, typename TYPE_ITEM>
void SystemStatus::getIteminReport(TYPE_REPORT& reportout,
                                   const TYPE_ITEM& c) const {
  reportout.clear();
  if (c.size() >= 1) {
...
bool SystemStatus::eventPosition(const UlpLocation& location,
  ...
  ret = setIteminReport(mCache.mLocation,
                        SystemStatusLocation(location, locationEx));
...
@return     true when successfully done
******************************************************************************/
bool SystemStatus::getReport(SystemStatusReports& report,
                             bool isLatestOnly) const {
  pthread_mutex_lock(&mMutexSystemStatus);
...
bool SystemStatus::setDefaultGnssEngineStates(void) {
  ...
  setDefaultIteminReport(mCache.mInjectedPosition,
                         SystemStatusInjectedPosition());
  ...
  setDefaultIteminReport(mCache.mPositionFailure,
                         SystemStatusPositionFailure());
