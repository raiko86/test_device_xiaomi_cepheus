/* Copyright (c) 2011-2014,2016-2017,2020 The Linux Foundation. All rights
 * reserved.
 *
...
const loc_param_s_type ContextBase::mGps_conf_table[] = {
    ...
    {"A_GLONASS_POS_PROTOCOL_SELECT", &mGps_conf.A_GLONASS_POS_PROTOCOL_SELECT,
     NULL, 'n'},
    ...
    {"NI_SUPL_DENY_ON_NFW_LOCKED", &mGps_conf.NI_SUPL_DENY_ON_NFW_LOCKED, NULL,
     'n'},
...
const loc_param_s_type ContextBase::mSap_conf_table[] = {
    ...
    {"SENSOR_ACCEL_BATCHES_PER_SEC", &mSap_conf.SENSOR_ACCEL_BATCHES_PER_SEC,
     NULL, 'n'},
    {"SENSOR_ACCEL_SAMPLES_PER_BATCH",
     &mSap_conf.SENSOR_ACCEL_SAMPLES_PER_BATCH, NULL, 'n'},
    {"SENSOR_GYRO_BATCHES_PER_SEC", &mSap_conf.SENSOR_GYRO_BATCHES_PER_SEC,
     NULL, 'n'},
    {"SENSOR_GYRO_SAMPLES_PER_BATCH", &mSap_conf.SENSOR_GYRO_SAMPLES_PER_BATCH,
     NULL, 'n'},
    ...
    {"SENSOR_ALGORITHM_CONFIG_MASK", &mSap_conf.SENSOR_ALGORITHM_CONFIG_MASK,
     NULL, 'n'}};
...
void ContextBase::readConfig() {
  ...
  if (!confReadDone) {
    ...
    /* inject supl config to modem with config values from config.xml or
     * gps.conf, default 1 */
    ...
    if (strncmp(mGps_conf.NMEA_REPORT_RATE, "1HZ",
                sizeof(mGps_conf.NMEA_REPORT_RATE)) == 0) {
    ...
    switch (getTargetGnssType(loc_get_target())) {
      ...
      case GNSS_AUTO:
        ...
        mGps_conf.CAPABILITIES &=
            ~(LOC_GPS_CAPABILITY_MSA | LOC_GPS_CAPABILITY_MSB);
        break;
      default:
        break;
    }
...
#define gpsConfMSB (uint32_t)0x2
  ...
  LOC_LOGV(
      "getCarrierCapabilities: CAPABILITIES %x, SUPL_MODE %x, carrier "
      "capabilities %x",
  ...
  if (TARGET_NO_GNSS != loc_get_target()) {
    if (NULL == (locApi = mLBSProxy->getLocApi(exMask, this))) {
      ...
      if ((handle = dlopen(libname, RTLD_NOW)) != NULL) {
        ...
        if (getter != NULL) {
          LOC_LOGD("%s:%d]: getter is not NULL of %s", __func__, __LINE__,
                   libname);
    ...
    if (ContextBase::isFeatureSupported(
      ...
      if (!isSapModeKnown) {
        ...
        if (strcmp(conf_feature_sap, "PREMIUM_ENV_AIDING") != 0) {
          uint8_t arrayIndex =
              LOC_SUPPORTED_FEATURE_MEASUREMENTS_CORRECTION >> 3;
