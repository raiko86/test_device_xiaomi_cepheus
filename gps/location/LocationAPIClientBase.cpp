...
LocationAPIControlClient::LocationAPIControlClient() : mEnabled(false) {
  ...
  locationControlCallbacks.responseCb =
      [this](LocationError error, uint32_t id) { onCtrlResponseCb(error, id); };
  ...
  mLocationControlAPI =
      LocationControlAPI::createInstance(locationControlCallbacks);
}
...
uint32_t LocationAPIControlClient::locAPIGnssDeleteAidingData(
    GnssAidingData& data) {
  ...
  if (mLocationControlAPI) {
    ...
    mRequestQueues[CTRL_REQUEST_DELETEAIDINGDATA].push(
        new GnssDeleteAidingDataRequest(*this));
...
uint32_t LocationAPIControlClient::locAPIEnable(
...
uint32_t LocationAPIControlClient::locAPIGnssUpdateConfig(GnssConfig config) {
  ...
  if (mLocationControlAPI) {
    if (mConfig.equals(config)) {
      ...
      if (nullptr != idArray) {
        if (nullptr !=
            mRequestQueues[CTRL_REQUEST_CONFIG_UPDATE].getSessionArrayPtr()) {
          mRequestQueues[CTRL_REQUEST_CONFIG_UPDATE].reset(idArray);
        }
        mRequestQueues[CTRL_REQUEST_CONFIG_UPDATE].push(
            new GnssUpdateConfigRequest(*this));
        retVal = LOCATION_ERROR_SUCCESS;
      }
    }
  }
...
uint32_t LocationAPIControlClient::locAPIGnssGetConfig(
  ...
  if (mLocationControlAPI) {
    ...
    if (nullptr != idArray) {
      if (nullptr !=
          mRequestQueues[CTRL_REQUEST_CONFIG_GET].getSessionArrayPtr()) {
        mRequestQueues[CTRL_REQUEST_CONFIG_GET].reset(idArray);
      }
      mRequestQueues[CTRL_REQUEST_CONFIG_GET].push(
          new GnssGetConfigRequest(*this));
      retVal = LOCATION_ERROR_SUCCESS;
    }
  }
  pthread_mutex_unlock(&mMutex);
...
void LocationAPIControlClient::onCtrlResponseCb(LocationError error,
                                                uint32_t id) {
...
void LocationAPIControlClient::onCtrlCollectiveResponseCb(size_t count,
                                                          LocationError* errors,
                                                          uint32_t* ids) {
  for (size_t i = 0; i < count; i++) {
    if (errors[i] != LOCATION_ERROR_SUCCESS) {
      LOC_LOGE("%s:%d] ERROR: %d ID: %d", __FUNCTION__, __LINE__, errors[i],
               ids[i]);
    } else {
      LOC_LOGV("%s:%d] SUCCESS: %d id: %d", __FUNCTION__, __LINE__, errors[i],
               ids[i]);
    }
  }
...
LocationAPIRequest* LocationAPIControlClient::getRequestBySession(
    uint32_t session) {
  pthread_mutex_lock(&mMutex);
...
LocationAPIRequest* LocationAPIControlClient::getRequestBySessionArrayPtr(
  ...
  if (mRequestQueues[CTRL_REQUEST_CONFIG_UPDATE].getSessionArrayPtr() ==
      sessionArrayPtr) {
    request = mRequestQueues[CTRL_REQUEST_CONFIG_UPDATE].pop();
  } else if (mRequestQueues[CTRL_REQUEST_CONFIG_GET].getSessionArrayPtr() ==
             sessionArrayPtr) {
...
void LocationAPIClientBase::locAPISetCallbacks(
  ...
  if (locationCallbacks.batchingStatusCb != nullptr) {
    ...
    locationCallbacks.batchingStatusCb =
        [this](BatchingStatusInfo batchStatus,
...
uint32_t LocationAPIClientBase::locAPIStartTracking(TrackingOptions& options) {
  ...
  if (mLocationAPI) {
    if (mTracking) {
      LOC_LOGW("%s:%d] Existing tracking session present", __FUNCTION__,
               __LINE__);
    } else {
      ...
      // until StartTrackingRequest is pushed into
      // mRequestQueues[REQUEST_TRACKING]
...
void LocationAPIClientBase::locAPIUpdateTrackingOptions(
  ...
  if (mLocationAPI) {
    ...
    session = mRequestQueues[REQUEST_TRACKING].getSession();
    if (session > 0) {
      mRequestQueues[REQUEST_TRACKING].push(
          new UpdateTrackingOptionsRequest(*this));
...
uint32_t LocationAPIClientBase::locAPIStartSession(uint32_t id,
  ...
  if (mLocationAPI) {
    if (mSessionBiDict.hasId(id)) {
      LOC_LOGE("%s:%d] session %d has already started.", __FUNCTION__, __LINE__,
               id);
      ...
      uint32_t session =
          ((sessionMode != SESSION_MODE_ON_FIX) ? batchingSession
                                                : trackingSession);
...
uint32_t LocationAPIClientBase::locAPIUpdateSessionOptions(
  ...
  if (mLocationAPI) {
    if (mSessionBiDict.hasId(id)) {
      ...
      if (sessionMode == SESSION_MODE_ON_FIX) {
        // we only add an UpdateTrackingOptionsRequest to
        // mRequestQueues[REQUEST_SESSION], even if this update request will
        // stop batching and then start tracking.
        mRequestQueues[REQUEST_SESSION].push(
            new UpdateTrackingOptionsRequest(*this));
        if (sMode == SESSION_MODE_ON_FIX) {
          ...
          // so we don't need to add a new request to
          // mRequestQueues[REQUEST_SESSION].
          mLocationAPI->stopBatching(batchingSession);
          batchingSession = 0;
          mRequestQueues[REQUEST_SESSION].setSession(batchingSession);
        ...
        // we only add an UpdateBatchingOptionsRequest to
        // mRequestQueues[REQUEST_SESSION], even if this update request will
        // stop tracking and then start batching.
        mRequestQueues[REQUEST_SESSION].push(
...
uint32_t LocationAPIClientBase::locAPIGetBatchedLocations(uint32_t id,
                                                          size_t count) {
  uint32_t retVal = LOCATION_ERROR_GENERAL_FAILURE;
  pthread_mutex_lock(&mMutex);
  if (mLocationAPI) {
    if (mSessionBiDict.hasId(id)) {
      ...
      if (entity.sessionMode != SESSION_MODE_ON_FIX) {
        uint32_t batchingSession = entity.batchingSession;
        mRequestQueues[REQUEST_SESSION].push(
            new GetBatchedLocationsRequest(*this));
        mLocationAPI->getBatchedLocations(batchingSession, count);
        retVal = LOCATION_ERROR_SUCCESS;
      } else {
        LOC_LOGE(
            "%s:%d] Unsupported for session id: %d, mode is "
            "SESSION_MODE_ON_FIX",
...
uint32_t LocationAPIClientBase::locAPIAddGeofences(size_t count, uint32_t* ids,
  ...
  if (mLocationAPI) {
    ...
    if (sessions) {
      LOC_LOGI("%s:%d] start new sessions: %p", __FUNCTION__, __LINE__,
               sessions);
      ...
      for (size_t i = 0; i < count; i++) {
        ...
        if (sessions[j] > 0) {
          GeofenceBreachTypeMask type =
              mGeofenceBiDict.getExtBySession(sessions[j]);
...
void LocationAPIClientBase::locAPIModifyGeofences(size_t count, uint32_t* ids,
  ...
  if (mLocationAPI) {
    ...
    if (mRequestQueues[REQUEST_GEOFENCE].getSession() == GEOFENCE_SESSION_ID) {
      ...
      if (j > 0) {
        mRequestQueues[REQUEST_GEOFENCE].push(
            new ModifyGeofencesRequest(*this));
...
void LocationAPIClientBase::locAPIResumeGeofences(
  ...
  if (mLocationAPI) {
    ...
    if (mRequestQueues[REQUEST_GEOFENCE].getSession() == GEOFENCE_SESSION_ID) {
      ...
      if (j > 0) {
        mRequestQueues[REQUEST_GEOFENCE].push(
            new ResumeGeofencesRequest(*this));
...
void LocationAPIClientBase::beforeGeofenceBreachCb(
  ...
  uint32_t* ids =
      (uint32_t*)malloc(sizeof(uint32_t) * geofenceBreachNotification.count);
...
void LocationAPIClientBase::beforeBatchingStatusCb(
    BatchingStatusInfo batchStatus, std::list<uint32_t>& tripCompletedList) {
  ...
  if (batchStatus.batchingStatus == BATCHING_STATUS_TRIP_COMPLETED) {
    for (auto itt = tripCompletedList.begin(); itt != tripCompletedList.end();
         itt++) {
...
void LocationAPIClientBase::onCollectiveResponseCb(size_t count,
                                                   LocationError* errors,
                                                   uint32_t* ids) {
  for (size_t i = 0; i < count; i++) {
    if (errors[i] != LOCATION_ERROR_SUCCESS) {
...
LocationAPIRequest* LocationAPIClientBase::getRequestBySession(
    uint32_t session) {
  pthread_mutex_lock(&mMutex);
  LocationAPIRequest* request = nullptr;
  for (int i = 0; i < REQUEST_MAX; i++) {
