...
typedef struct {
  // bit mask of the adpaters that we need to wait for the
  // removeClientCompleteCallback before we invoke the registered
...
void LocationAPI::onRemoveClientCompleteCb(
    LocationAdapterTypeMask adapterType) {
...
LocationAPI* LocationAPI::createInstance(LocationCallbacks& locationCallbacks) {
  ...
  if (isGnssClient(locationCallbacks)) {
    if (NULL == gData.gnssInterface && !gGnssLoadFailed) {
      gData.gnssInterface = (GnssInterface*)
          loadLocationInterface<GnssInterface, getGnssInterface>(
...
void LocationAPI::destroy(
  ...
  if (it != gData.clientData.end()) {
    ...
    LOC_LOGe(
        "removeFromGnssInf: %d, removeFromBatchingInf: %d, "
        "removeFromGeofenceInf: %d,"
        "needToWait: %d",
        removeFromGnssInf, removeFromBatchingInf, removeFromGeofenceInf,
        needToWait);
    ...
    if ((NULL != destroyCompleteCb) && (true == needToWait)) {
      ...
      // record down from which adapter we need to wait for the destroy complete
      // callback only when we have received all the needed callbacks from all
      // the associated stacks, we shall notify the client.
      ...
      LOC_LOGi("destroy data stored in the map: 0x%x",
               destroyCbData.waitAdapterMask);
    }
...
LocationAPI::LocationAPI() { LOC_LOGD("LOCATION API CONSTRUCTOR"); }

// private destructor
LocationAPI::~LocationAPI() { LOC_LOGD("LOCATION API DESTRUCTOR"); }
...
uint32_t LocationAPI::startTracking(TrackingOptions& trackingOptions) {
  ...
  if (it != gData.clientData.end()) {
    if (NULL != gData.gnssInterface) {
      ...
      LOC_LOGE(
          "%s:%d]: No gnss interface available for Location API client %p ",
          __func__, __LINE__, this);
    }
  } else {
    LOC_LOGE("%s:%d]: Location API client %p not found in client data",
...
void LocationAPI::updateTrackingOptions(uint32_t id,
...
uint32_t LocationAPI::startBatching(BatchingOptions& batchingOptions) {
  ...
  if (NULL != gData.batchingInterface) {
    ...
    LOC_LOGE(
        "%s:%d]: No batching interface available for Location API client %p ",
        __func__, __LINE__, this);
  }
...
void LocationAPI::updateBatchingOptions(uint32_t id,
...
uint32_t* LocationAPI::addGeofences(size_t count, GeofenceOption* options,
                                    GeofenceInfo* info) {
  uint32_t* ids = NULL;
  pthread_mutex_lock(&gDataMutex);
  ...
  if (gData.geofenceInterface != NULL) {
    ...
    LOC_LOGE(
        "%s:%d]: No geofence interface available for Location API client %p ",
        __func__, __LINE__, this);
  }
...
void LocationAPI::modifyGeofences(size_t count, uint32_t* ids,
...
LocationControlAPI* LocationControlAPI::createInstance(
  ...
  if (nullptr != locationControlCallbacks.responseCb &&
      NULL == gData.controlAPI) {
    if (NULL == gData.gnssInterface && !gGnssLoadFailed) {
      gData.gnssInterface = (GnssInterface*)
          loadLocationInterface<GnssInterface, getGnssInterface>(
              "libgnss.so", "getGnssInterface");
...
void LocationControlAPI::destroy() { delete this; }
...
uint32_t LocationControlAPI::enable(LocationTechnologyType techType) {
  ...
  if (gData.gnssInterface != NULL) {
    ...
    LOC_LOGE(
        "%s:%d]: No gnss interface available for Location Control API client "
        "%p ",
        __func__, __LINE__, this);
  }
...
uint32_t LocationControlAPI::configConstellationSecondaryBand(
  ...
  if (gData.gnssInterface != NULL) {
    id =
        gData.gnssInterface->gnssUpdateSecondaryBandConfig(secondaryBandConfig);
  } else {
    LOC_LOGe("No gnss interface available for Location Control API");
  }
...
uint32_t LocationControlAPI::configConstrainedTimeUncertainty(
  ...
  if (gData.gnssInterface != NULL) {
    id = gData.gnssInterface->setConstrainedTunc(enable, tuncThreshold,
                                                 energyBudget);
  } else {
    LOC_LOGe("No gnss interface available for Location Control API");
  }
...
uint32_t LocationControlAPI::configLeverArm(
...
uint32_t LocationControlAPI::configRobustLocation(bool enable,
                                                  bool enableForE911) {
  uint32_t id = 0;
  pthread_mutex_lock(&gDataMutex);
