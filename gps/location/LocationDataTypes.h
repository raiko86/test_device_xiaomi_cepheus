...
typedef enum {
  LOCATION_HAS_LAT_LONG_BIT =
      (1 << 0),  // location has valid latitude and longitude
  ...
  LOCATION_HAS_VERTICAL_ACCURACY_BIT =
      (1 << 5),  // location has valid vertical accuracy
  LOCATION_HAS_SPEED_ACCURACY_BIT =
      (1 << 6),  // location has valid speed accuracy
  LOCATION_HAS_BEARING_ACCURACY_BIT =
      (1 << 7),                        // location has valid bearing accuracy
  LOCATION_HAS_SPOOF_MASK = (1 << 8),  // location has valid spoof mask
  LOCATION_HAS_ELAPSED_REAL_TIME =
      (1 << 9),  // location has valid elapsed real time
  LOCATION_HAS_CONFORMITY_INDEX_BIT =
      (1 << 10),  // location has valid conformity index
} LocationFlagsBits;
...
typedef enum {
  ...
  LOCATION_NAVIGATION_DATA_SPOOFED =
      (1 << 2),  // location navigation data spoofed
} LocationSpoofBits;
...
typedef enum {
  LOCATION_NAV_DATA_HAS_LONG_ACCEL_BIT =
      (1 << 0),  // Navigation data has Forward Acceleration
  LOCATION_NAV_DATA_HAS_LAT_ACCEL_BIT =
      (1 << 1),  // Navigation data has Sideward Acceleration
  LOCATION_NAV_DATA_HAS_VERT_ACCEL_BIT =
      (1 << 2),  // Navigation data has Vertical Acceleration
  LOCATION_NAV_DATA_HAS_YAW_RATE_BIT =
      (1 << 3),  // Navigation data has Heading Rate
...
typedef enum {
  GNSS_LOCATION_INFO_ALTITUDE_MEAN_SEA_LEVEL_BIT =
      (1 << 0),                           // altitude mean sea level
  ...
  GNSS_LOCATION_INFO_HOR_ACCURACY_ELIP_SEMI_MAJOR_BIT =
      (1 << 5),  // elipsode semi major
  GNSS_LOCATION_INFO_HOR_ACCURACY_ELIP_SEMI_MINOR_BIT =
      (1 << 6),  // elipsode semi minor
  GNSS_LOCATION_INFO_HOR_ACCURACY_ELIP_AZIMUTH_BIT =
      (1 << 7),  // accuracy elipsode azimuth
  GNSS_LOCATION_INFO_GNSS_SV_USED_DATA_BIT =
      (1 << 8),  // svUsedInPosition,
                 //       numOfMeasReceived
                 //       and measUsageInfo
  GNSS_LOCATION_INFO_NAV_SOLUTION_MASK_BIT = (1 << 9),  // navSolutionMask
  GNSS_LOCATION_INFO_SV_SOURCE_INFO_BIT = (1 << 10),    // LocSvInfoSource
  GNSS_LOCATION_INFO_POS_DYNAMICS_DATA_BIT =
      (1 << 11),                               // position dynamics data &
  ...
  GNSS_LOCATION_INFO_NORTH_VEL_UNC_BIT =
      (1 << 18),  // North Velocity Uncertainty
  ...
  GNSS_LOCATION_INFO_NUM_SV_USED_IN_POSITION_BIT =
      (1 << 23),  // number of SV used in position
  GNSS_LOCATION_INFO_CALIBRATION_CONFIDENCE_BIT =
      (1 << 24),  // sensor cal confidence
  ...
  GNSS_LOCATION_INFO_ENU_VELOCITY_VRP_BASED_BIT =
      (1 << 30),  // VRP-based east/north/up vel
  GNSS_LOCATION_INFO_DR_SOLUTION_STATUS_MASK_BIT =
      (1ULL << 31),  // DR solution status
...
typedef enum {
  ...
  // supports startTracking/startBatching API with LocationOptions.mode of MSB
  // (Ms Based)
  LOCATION_CAPABILITIES_GNSS_MSB_BIT = (1 << 6),
  // supports startTracking/startBatching API with LocationOptions.mode of MSA
  // (MS Assisted)
...
enum {
  ...
  GNSS_CONFIG_GPS_LOCK_MO,  // gps mobile originated (MO) is locked when GPS is
                            // disabled
  GNSS_CONFIG_GPS_LOCK_NI,  // gps network initiated (NI) is locked when GPS is
                            // disabled
  GNSS_CONFIG_GPS_LOCK_MO_AND_NI,  // gps MO and NI is locked when GPS is
                                   // disabled
};
...
typedef enum {
  ...
  GNSS_CONFIG_LPP_PROFILE_USER_PLANE_BIT =
      (1 << 0),  // LPP User Plane (UP) on LTE
  GNSS_CONFIG_LPP_PROFILE_CONTROL_PLANE_BIT =
      (1 << 1),  // LPP_Control_Plane (CP)
  GNSS_CONFIG_LPP_PROFILE_USER_PLANE_OVER_NR5G_SA_BIT =
      (1 << 2),  // LPP User Plane (UP) on LTE
  GNSS_CONFIG_LPP_PROFILE_CONTROL_PLANE_OVER_NR5G_SA_BIT =
      (1 << 3),  // LPP_Control_Plane (CP)
...
typedef enum {
  BATCHING_MODE_ROUTINE =
      0,  // positions are reported when batched positions memory is full
  BATCHING_MODE_TRIP,  // positions are reported when a certain distance is
                       // covered
  BATCHING_MODE_NO_AUTO_REPORT  // no report of positions automatically, instead
                                // queried on demand
} BatchingMode;
...
#define GNSS_SIGNAL_TYPE_MASK_ALL                                              \
  (GNSS_SIGNAL_GPS_L1CA | GNSS_SIGNAL_GPS_L1C | GNSS_SIGNAL_GPS_L2 |           \
   ...
   GNSS_SIGNAL_QZSS_L1S | GNSS_SIGNAL_QZSS_L2 | GNSS_SIGNAL_QZSS_L5 |          \
...
typedef enum {
  ...
  GNSS_LOC_MAX_NUMBER_OF_SIGNAL_TYPES =
      20 /**<  Maximum number of signal types */
...
typedef struct {
  ...
  GnssAidingDataSvTypeMask
      svTypeMask;  // bitwise OR of GnssAidingDataSvTypeBits
} GnssAidingDataSv;
...
typedef enum {
  ...
  GNSS_AIDING_DATA_COMMON_FREQ_BIAS_EST_BIT =
      (1 << 4),                                   // frequency bias estimate
...
typedef enum {
  DR_ENGINE_AIDING_DATA_CALIBRATION_BIT =
      (1 << 0),  // Calibration data for DRE engine
...
typedef struct {
  ...
  PositioningEngineMask
      posEngineMask;  // engines to perform the delete operation on.
} GnssAidingData;

typedef uint16_t DrCalibrationStatusMask;
typedef enum {
  // Indicate that roll calibration is needed. Need to take more turns on level
  // ground
  DR_ROLL_CALIBRATION_NEEDED = (1 << 0),
  // Indicate that pitch calibration is needed. Need to take more turns on level
  // ground
  DR_PITCH_CALIBRATION_NEEDED = (1 << 1),
  // Indicate that yaw calibration is needed. Need to accelerate in a straight
  // line
  DR_YAW_CALIBRATION_NEEDED = (1 << 2),
  // Indicate that odo calibration is needed. Need to accelerate in a straight
  // line
  DR_ODO_CALIBRATION_NEEDED = (1 << 3),
  // Indicate that gyro calibration is needed. Need to take more turns on level
  // ground
...
typedef struct {
  ...
  LocationFlagsMask
      flags;  // bitwise OR of LocationFlagsBits to mark which params are valid
  uint64_t timestamp;      // UTC timestamp for location fix, milliseconds since
                           // January 1, 1970
...
struct LocationOptions {
  ...
  uint32_t minDistance;  // in meters. if minDistance > 0,
                         // gnssSvCallback/gnssNmeaCallback/
...
typedef struct {
  ...
  GeofenceBreachTypeMask
      breachTypeMask;       // bitwise OR of GeofenceBreachTypeBits
...
typedef struct {
  GnssLocationPosDataMask
      bodyFrameDataMask;  // Contains Body frame LocPosDataMask bits
...
typedef struct {
  GnssLocationPosDataMaskExt
      bodyFrameDataMask;  // Contains Ext Body frame LocPosDataMask bits
  ...
  float
      rollRate;  // Roll rate of body frame. Clockwise positive. (radian/second)
  float rollRateUnc;  // Uncertainty of Roll rate, 68% confidence level
                      // (radian/second)
...
typedef struct {
  ...
  (UTC). For GAL: Calculated from 00:00 UT on Sunday August 22, 1999 (midnight
...
typedef struct {
  ...
  GnssLocationInfoFlagMask
      flags;  // bitwise OR of GnssLocationInfoBits for param validity
  ...
  float
      horUncEllipseSemiMinor;  // horizontal elliptical accuracy semi-minor axis
  ...
  GnssLocationSvUsedInPosition
      svUsedInPosition;  // Gnss sv used in position data
  GnssLocationNavSolutionMask
      navSolutionMask;  // Nav solution mask to indicate sbas corrections
  GnssLocationPositionDynamics
      bodyFrameData;              // Body Frame Dynamics: 4wayAcceleration and
  ...
  GnssLocationPositionDynamicsExt
      bodyFrameDataExt;  // Additional Body Frame Dynamics
...
typedef struct {
  ...
  char requestor[GNSS_NI_REQUESTOR_MAX];  // the requestor that is making the
                                          // request
...
typedef struct {
  ...
  uint16_t svId;
  GnssSvType
      type;  // type of SV (GPS, SBAS, GLONASS, QZSS, BEIDOU, GALILEO, NAVIC)
...
typedef struct {
  ...
  GnssMeasurementsStateMask
      stateMask;  // bitwise OR of GnssMeasurementsStateBits
  ...
  GnssMeasurementsAdrStateMask
      adrStateMask;  // bitwise OR of GnssMeasurementsAdrStateBits
  double adrMeters;
...
typedef struct {
  ...
  GnssMeasurementsClockFlagsMask
      flags;  // bitwise OR of GnssMeasurementsClockFlagsBits
  int16_t leapSecond;
  int64_t timeNs;
...
typedef struct {
  ...
  GnssDataMask
      gnssDataMask[GNSS_LOC_MAX_NUMBER_OF_SIGNAL_TYPES];  // bitwise OR of
                                                          // GnssDataBits
...
inline bool operator==(GnssSvIdSource const& left,
                       GnssSvIdSource const& right) {
...
#define GNSS_SV_TYPES_MASK_ALL                                \
  (GNSS_SV_TYPES_MASK_GPS_BIT | GNSS_SV_TYPES_MASK_GLO_BIT |  \
   GNSS_SV_TYPES_MASK_BDS_BIT | GNSS_SV_TYPES_MASK_QZSS_BIT | \
   GNSS_SV_TYPES_MASK_GAL_BIT | GNSS_SV_TYPES_MASK_NAVIC_BIT)
...
struct GnssConfig {
  ...
  GnssConfigFlagsMask flags;  // bitwise OR of GnssConfigFlagsBits to mark which
                              // params are valid
...
struct LeverArmConfigInfo {
  ...
  // Lever arm regarding GNSS Antenna w.r.t the origin at the IMU
  // (inertial measurement unit) for VEPP (vision enhanced precise position
  // engine)
...
typedef std::function<void(
    LocationCapabilitiesMask capabilitiesMask  // bitwise OR of
                                               // LocationCapabilitiesBits
    )>
    capabilitiesCallback;

/* Used by tracking, batching, and miscellanous APIs
   responseCallback is called for every Tracking, Batching API, and Miscellanous
   API */
typedef std::function<void(
...
/* Used by APIs that gets more than one LocationError in it's response
   ...
   ids array and LocationError array are only valid until
...
/* Gives default combined location information from all engines and
   ...
   broadcasted to all clients, no matter if a session has started by client */
typedef std::function<void(uint32_t count, GnssLocationInfoNotification*
                                               engineLocationInfoNotification)>
    engineLocationsInfoCallback;

/* Used for addGeofences API, optional can be NULL
   geofenceBreachCallback is called when any number of geofences have a state
   change */
typedef std::function<void(
...
/* Used for addGeofences API, optional can be NULL
       geofenceStatusCallback is called when any number of geofences have a
   status change */
typedef std::function<void(
...
/* Gives GNSS SV information, optional can be NULL
    ...
    broadcasted to all clients, no matter if a session has started by client */
typedef std::function<void(GnssSvNotification gnssSvNotification)>
    gnssSvCallback;

/* Gives GNSS NMEA data, optional can be NULL
    gnssNmeaCallback is called only during a tracking session
    broadcasted to all clients, no matter if a session has started by client */
typedef std::function<void(GnssNmeaNotification gnssNmeaNotification)>
    gnssNmeaCallback;

/* Gives GNSS data, optional can be NULL
    gnssDataCallback is called only during a tracking session
    broadcasted to all clients, no matter if a session has started by client */
typedef std::function<void(GnssDataNotification gnssDataNotification)>
    gnssDataCallback;
...
typedef std::function<void(uint32_t session_id, const GnssConfig& config)>
    gnssConfigCallback;

/* LocationSystemInfoCb is for receiving rare occuring location
   system information update. optional, can be NULL.
*/
typedef std::function<void(LocationSystemInfo locationSystemInfo)>
