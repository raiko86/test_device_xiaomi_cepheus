...
ssize_t Sock::send(const void* buf, uint32_t len, int flags,
                   const struct sockaddr* destAddr, socklen_t addrlen) const {
  ssize_t rtv = -1;
  SOCK_OP_AND_LOG(buf, len, isValid(), rtv,
                  sendto(buf, len, flags, destAddr, addrlen));
  return rtv;
}
ssize_t Sock::recv(const LocIpcRecver& recver,
...
ssize_t Sock::sendto(const void* buf, size_t len, int flags,
                     const struct sockaddr* destAddr, socklen_t addrlen) const {
  ssize_t rtv = -1;
  if (len <= mMaxTxSize) {
    ...
    if (rtv > 0) {
      for (size_t offset = 0; offset < len && rtv > 0; offset += rtv) {
        rtv = ::sendto(mSid, (char*)buf + offset,
                       min(len - offset, (size_t)mMaxTxSize), flags, destAddr,
                       addrlen);
      }
  ...
  if (nBytes > 0) {
    if (strncmp(msg.data(), MSG_ABORT, sizeof(MSG_ABORT)) == 0) {
      ...
      for (size_t msgLenReceived = 0; (msgLenReceived < msgLen) && (nBytes > 0);
        ...
        nBytes =
            ::recvfrom(sid, &(msg[msgLenReceived]), msg.size() - msgLenReceived,
...
ssize_t Sock::sendAbort(int flags, const struct sockaddr* destAddr,
                        socklen_t addrlen) {
...
class LocIpcLocalRecver : public LocIpcLocalSender, public LocIpcRecver {
 ...
 public:
  inline LocIpcLocalRecver(const shared_ptr<ILocIpcListener>& listener,
                           const char* name)
    ...
    umask(0157);
    if (mSock->isValid() &&
        ::bind(mSock->mSid, (struct sockaddr*)&mAddr, sizeof(mAddr)) < 0) {
...
class LocIpcInetSender : public LocIpcSender {
 ...
 public:
  ...
  inline LocIpcInetSender(const char* name, int32_t port, int sockType)
      : LocIpcSender(),
        mSockType(sockType),
        mSock(make_shared<Sock>(
            (nullptr == name) ? -1 : (::socket(AF_INET, mSockType, 0)))),
    ...
    if (mSock != nullptr && mSock->isValid() && nullptr != name) {
      ...
      if (nullptr != hp) {
        memcpy((char*)&(mAddr.sin_addr.s_addr), hp->h_addr_list[0],
               hp->h_length);
      }
    }
  }

  unique_ptr<LocIpcRecver> getRecver(
      const shared_ptr<ILocIpcListener>& listener) override {
...
class LocIpcInetRecver : public LocIpcInetSender, public LocIpcRecver {
 ...
 public:
  inline LocIpcInetRecver(const shared_ptr<ILocIpcListener>& listener,
                          const char* name, int32_t port, int sockType)
      : LocIpcInetSender(name, port, sockType),
        LocIpcRecver(listener, *this),
        mPort(port) {
    if (mSock->isValid() &&
        ::bind(mSock->mSid, (struct sockaddr*)&mAddr, sizeof(mAddr)) < 0) {
      LOC_LOGe("bind socket error. sock fd: %d, reason: %s", mSock->mSid,
               strerror(errno));
      mSock->close();
    }
  }
  ...
  inline virtual void abort() const override {
    if (isSendable()) {
      ...
      mSock->sendAbort(0, (struct sockaddr*)&loopBackAddr,
                       sizeof(loopBackAddr));
    }
  }
  inline virtual unique_ptr<LocIpcSender> getLastSender() const override {
    return make_unique<LocIpcInetSender>(
        static_cast<const LocIpcInetSender&>(*this));
  }
};
...
class LocIpcInetTcpRecver : public LocIpcInetRecver {
  ...
  inline virtual ssize_t recv() const override {
    ...
    if (-1 == mConnFd && mSock->isValid()) {
      if (::listen(mSock->mSid, 3) < 0 ||
          (mConnFd = accept(mSock->mSid, (struct sockaddr*)&mAddr, &size)) <
              0) {
        mSock->close();
        mConnFd = -1;
      }
 ...
 public:
  inline LocIpcInetTcpRecver(const shared_ptr<ILocIpcListener>& listener,
                             const char* name, int32_t port)
...
class LocIpcInetUdpRecver : public LocIpcInetRecver {
 ...
 public:
  inline LocIpcInetUdpRecver(const shared_ptr<ILocIpcListener>& listener,
                             const char* name, int32_t port)
...
class LocIpcRunnable : public LocRunnable {
 ...
 public:
  inline LocIpcRunnable(LocIpc& locIpc, unique_ptr<LocIpcRecver>& ipcRecver)
      : mAbortCalled(false), mLocIpc(locIpc), mIpcRecver(move(ipcRecver)) {}
  inline bool run() override {
    if (mIpcRecver != nullptr) {
      mLocIpc.startBlockingListening(*(mIpcRecver.get()));
      if (!mAbortCalled) {
        LOC_LOGw(
            "startListeningBlocking() returned w/o stopBlockingListening() "
            "called");
      }
    }
...
bool LocIpc::send(LocIpcSender& sender, const uint8_t data[], uint32_t length,
                  int32_t msgId) {
  return sender.sendData(data, length, msgId);
}

shared_ptr<LocIpcSender> LocIpc::getLocIpcLocalSender(
    const char* localSockName) {
  return make_shared<LocIpcLocalSender>(localSockName);
}
unique_ptr<LocIpcRecver> LocIpc::getLocIpcLocalRecver(
...
shared_ptr<LocIpcSender> LocIpc::getLocIpcQrtrSender(int service,
                                                     int instance) {
...
unique_ptr<LocIpcRecver> LocIpc::getLocIpcQrtrRecver(
    const shared_ptr<ILocIpcListener>& listener, int service, int instance,
    const shared_ptr<LocIpcQrtrWatcher>& watcher) {
  typedef unique_ptr<LocIpcRecver> (*creator_t)(
      const shared_ptr<ILocIpcListener>&, int, int,
      const shared_ptr<LocIpcQrtrWatcher>& watcher);
  static creator_t creator = (creator_t)dlGetSymFromLib(
      sLibQrtrHandle, sLibQrtrName,
#ifdef USE_GLIB
      "_ZN8loc_util22createLocIpcQrtrRecverERKSt10shared_ptrINS_"
      "15ILocIpcListenerEEiiRKS0_INS_17LocIpcQrtrWatcherEE");
#else
      "_ZN8loc_util22createLocIpcQrtrRecverERKNSt3__110shared_ptrINS_"
      "15ILocIpcListenerEEEiiRKNS1_INS_17LocIpcQrtrWatcherEEE");
#endif
  return (nullptr == creator) ? nullptr
                              : creator(listener, service, instance, watcher);
}
shared_ptr<LocIpcSender> LocIpc::getLocIpcInetTcpSender(const char* serverName,
                                                        int32_t port) {
  return make_shared<LocIpcInetTcpSender>(serverName, port);
}
unique_ptr<LocIpcRecver> LocIpc::getLocIpcInetTcpRecver(
...
LocIpc::getLocIpcQmiLocServiceSenderRecverPair(
  ...
  return (nullptr == creator)
             ? make_pair<shared_ptr<LocIpcSender>, unique_ptr<LocIpcRecver>>(
                   nullptr, nullptr)
