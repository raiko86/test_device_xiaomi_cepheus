...
/**
 * GPS HAL schedules fixes for LOC_GPS_POSITION_RECURRENCE_PERIODIC mode. If
 * this is not set, then the framework will use 1000ms for min_interval and will
 * start and call start() and stop() to schedule the GPS.
 */
...
typedef struct {
   ...
   * + 100 i.e. report an FCN of -7 as 93, FCN of 0 as 100, and FCN of +6 as
   * 106.
   * - QZSS:    193-200
   * - Galileo: 1-36
   * - Beidou:  1-37
 ...
 */
typedef void (*loc_gps_nmea_callback)(LocGpsUtcTime timestamp, const char* nmea,
                                      int length);
 ...
 * framework.
 */
typedef pthread_t (*loc_gps_create_thread)(const char* name,
...
typedef struct {
   ...
   * LOC_GPS_POSITION_MODE_MS_BASED if LOC_GPS_POSITION_MODE_MS_ASSISTED is
   * passed in, and LOC_GPS_POSITION_MODE_MS_BASED is supported.
   */
  int (*set_position_mode)(LocGpsPositionMode mode,
 ...
 * The Fingerprint is a digest of the DER Certificate that uniquely identifies
 * it.
 */
typedef struct {
  u_char data[20];
...
typedef struct {
  ...
  /**
   * Installs a set of Certificates used for SUPL connections to the AGPS
   * server. If needed the HAL should find out internally any certificates that
   * need to be removed to accommodate the certificates to install. The
   * certificates installed represent a full set of valid certificates needed to
   * connect to AGPS SUPL servers. The list of certificates is required, and all
   * must be available at the same time, when trying to establish a connection
   * with the AGPS Server.
   *
   * Parameters:
   *      certificates - A pointer to an array of DER encoded certificates that
   * are need to be installed in the HAL. length - The number of certificates to
   * install. Returns: LOC_AGPS_CERTIFICATE_OPERATION_SUCCESS if the operation
   * is completed successfully LOC_AGPS_CERTIFICATE_ERROR_TOO_MANY_CERTIFICATES
   * if the HAL cannot store the number of certificates attempted to be
   * installed, the state of the certificates stored should remain the same as
   * before on this error case.
   *
   * IMPORTANT:
   *      If needed the HAL should find out internally the set of certificates
   * that need to be removed to accommodate the certificates to install.
   */
  int (*install_certificates)(const LocDerEncodedCertificate* certificates,
                              size_t length);

  /**
   * Notifies the HAL that a list of certificates used for SUPL connections are
   * revoked. It is expected that the given set of certificates is removed from
   * the internal store of the HAL.
   *
   * Parameters:
   *      fingerprints - A pointer to an array of SHA1 Fingerprints to identify
   * the set of certificates to revoke. length - The number of fingerprints
   * provided. Returns: LOC_AGPS_CERTIFICATE_OPERATION_SUCCESS if the operation
   * is completed successfully.
   *
   * IMPORTANT:
   *      If any of the certificates provided (through its fingerprint) is not
   * known by the HAL, it should be ignored and continue revoking/deleting the
   * rest of them.
   */
  int (*revoke_certificates)(const LocSha1CertificateFingerprint* fingerprints,
                             size_t length);
...
typedef struct {
  ...
  /**
   * Notification/verification options, combinations of LocGpsNiNotifyFlags
   * constants
   */
...
typedef struct {
   ...
   */
  void (*set_ref_location)(const LocAGpsRefLocation* agps_reflocation,
                           size_t sz_struct);
  /**
   * Sets the set ID.
   */
   ...
   */
  void (*update_network_state)(int connected, int type, int roaming,
 ...
 *
 * When the GPS system is unavailable, loc_gps_geofence_status_callback should
 * be called to inform the upper layers of the same. Similarly, when it becomes
 ...
 *      transition  - Can be one of LOC_GPS_GEOFENCE_ENTERED,
 * LOC_GPS_GEOFENCE_EXITED, LOC_GPS_GEOFENCE_UNCERTAIN. timestamp   - Timestamp
 ...
 */
typedef void (*loc_gps_geofence_transition_callback)(int32_t geofence_id,
                                                     LocGpsLocation* location,
                                                     int32_t transition,
                                                     LocGpsUtcTime timestamp);

/**
 * The callback associated with the availability of the GPS system for
 * geofencing monitoring. If the GPS system determines that it cannot monitor
 * geofences because of lack of reliability or unavailability of the GPS
 * signals, it will call this callback with LOC_GPS_GEOFENCE_UNAVAILABLE
 * parameter.
 *
 * Parameters:
 ...
 */
typedef void (*loc_gps_geofence_status_callback)(int32_t status,
 ...
 *          LOC_GPS_GEOFENCE_ERROR_TOO_MANY_GEOFENCES  - geofence limit has been
 * reached. LOC_GPS_GEOFENCE_ERROR_ID_EXISTS  - geofence with id already exists
 *          LOC_GPS_GEOFENCE_ERROR_INVALID_TRANSITION - the monitorTransition
 * contains an invalid transition LOC_GPS_GEOFENCE_ERROR_GENERIC - for other
 * errors.
 */
typedef void (*loc_gps_geofence_add_callback)(int32_t geofence_id,
                                              int32_t status);
 ...
 */
typedef void (*loc_gps_geofence_remove_callback)(int32_t geofence_id,
                                                 int32_t status);
 ...
 *          LOC_GPS_GEOFENCE_ERROR_GENERIC for others.
 */
typedef void (*loc_gps_geofence_pause_callback)(int32_t geofence_id,
                                                int32_t status);
 ...
 *          LOC_GPS_GEOFENCE_ERROR_GENERIC for others.
 */
typedef void (*loc_gps_geofence_resume_callback)(int32_t geofence_id,
                                                 int32_t status);
...
typedef struct {
   ...
   * LOC_GNSS_CLOCK_HAS_LEAP_SECOND.
   */
  int16_t leap_second;
   ...
   * bias_uncertainty_ns
   *
   * If the data is available 'flags' must contain LOC_GNSS_CLOCK_HAS_BIAS. If
   * GPS has computed a position fix. This value is mandatory if the receiver
   ...
   *
   * If the data is available 'flags' must contain
   * LOC_GNSS_CLOCK_HAS_BIAS_UNCERTAINTY. This value is mandatory if the
   * receiver has estimated GPS time.
   */
  double bias_uncertainty_ns;
   ...
   * second). The uncertainty is represented as an absolute (single sided)
   * value.
   *
   * If the data is available 'flags' must contain
   * LOC_GNSS_CLOCK_HAS_DRIFT_UNCERTAINTY. If GPS has computed a position fix
   * this field is mandatory and must be populated.
   */
  double drift_uncertainty_nsps;
...
typedef struct {
   ...
   * The reference receiver's time is specified by LocGpsData::clock::time_ns
   * and should be interpreted in the same way as indicated by
   * LocGpsClock::type.
   *
   ...
   * sub-nanosecond accuracy. This is a mandatory value.
   */
  double time_offset_ns;

  /**
   * Per satellite sync state. It represents the current sync state for the
   * associated satellite. Based on the sync state, the 'received GPS tow' field
   ...
   * valid range for this field can be: Searching       : [ 0       ]   :
   * LOC_GNSS_MEASUREMENT_STATE_UNKNOWN C/A code lock   : [ 0   1ms ]   :
   * LOC_GNSS_MEASUREMENT_STATE_CODE_LOCK is set Bit sync        : [ 0  20ms ]
   * : LOC_GNSS_MEASUREMENT_STATE_BIT_SYNC is set Subframe sync   : [ 0    6s ]
   * : LOC_GNSS_MEASUREMENT_STATE_SUBFRAME_SYNC is set TOW decoded     : [ 0
   * 1week ]   : LOC_GNSS_MEASUREMENT_STATE_TOW_DECODED is set
   *
   *   Note well: if there is any ambiguity in integer millisecond,
   *   LOC_GNSS_MEASUREMENT_STATE_MSEC_AMBIGUOUS should be set accordingly, in
   * the 'state' field.
   *
   *   This value must be populated if 'state' !=
   * LOC_GNSS_MEASUREMENT_STATE_UNKNOWN.
   *
   * For Glonass, this is:
   ...
   * valid range for this field can be: Searching       : [ 0       ]   :
   * LOC_GNSS_MEASUREMENT_STATE_UNKNOWN C/A code lock   : [ 0   1ms ]   :
   * LOC_GNSS_MEASUREMENT_STATE_CODE_LOCK is set Symbol sync     : [ 0  10ms ]
   * : LOC_GNSS_MEASUREMENT_STATE_SYMBOL_SYNC is set Bit sync        : [ 0  20ms
   * ]   : LOC_GNSS_MEASUREMENT_STATE_BIT_SYNC is set String sync     : [ 0 2s ]
   * : LOC_GNSS_MEASUREMENT_STATE_GLO_STRING_SYNC is set Time of day     : [ 0
   * 1day ]   : LOC_GNSS_MEASUREMENT_STATE_GLO_TOD_DECODED is set
   *
   * For Beidou, this is:
   ...
   * valid range for this field can be: Searching    : [ 0       ] :
   * LOC_GNSS_MEASUREMENT_STATE_UNKNOWN C/A code lock: [ 0   1ms ] :
   * LOC_GNSS_MEASUREMENT_STATE_CODE_LOCK is set Bit sync (D2): [ 0   2ms ] :
   * LOC_GNSS_MEASUREMENT_STATE_BDS_D2_BIT_SYNC is set Bit sync (D1): [ 0  20ms
   * ] : LOC_GNSS_MEASUREMENT_STATE_BIT_SYNC is set Subframe (D2): [ 0  0.6s ] :
   * LOC_GNSS_MEASUREMENT_STATE_BDS_D2_SUBFRAME_SYNC is set Subframe (D1): [ 0
   * 6s ] : LOC_GNSS_MEASUREMENT_STATE_SUBFRAME_SYNC is set Time of week : [ 0
   * 1week ] : LOC_GNSS_MEASUREMENT_STATE_TOW_DECODED is set
   *
   * For Galileo, this is:
   *   Received Galileo time of week, at the measurement time in nanoseconds.
   *
   *     E1BC code lock   : [ 0   4ms ]   :
   * LOC_GNSS_MEASUREMENT_STATE_GAL_E1BC_CODE_LOCK is set E1C 2nd code lock: [ 0
   * 100ms ]   : LOC_GNSS_MEASUREMENT_STATE_GAL_E1C_2ND_CODE_LOCK is set
   *
   *     E1B page    : [ 0    2s ] :
   * LOC_GNSS_MEASUREMENT_STATE_GAL_E1B_PAGE_SYNC is set Time of week: [ 0 1week
   * ] : LOC_GNSS_MEASUREMENT_STATE_TOW_DECODED is set
   *
   * For SBAS, this is:
   ...
   *     Symbol sync  : [ 0 2ms ] : LOC_GNSS_MEASUREMENT_STATE_SYMBOL_SYNC is
   * set Message      : [ 0  1s ] : LOC_GNSS_MEASUREMENT_STATE_SBAS_SYNC is set
   */
   ...
   *
   * This value must be populated if 'state' !=
   * LOC_GPS_MEASUREMENT_STATE_UNKNOWN.
   */
   ...
   * provide LocGpsClock's 'drift' field as well (When providing the uncorrected
   * pseudorange rate, do not apply the corrections described above.)
   *
   ...
   * sign of 'doppler shift' is given by the equation: pseudorange rate = -k *
  ...
  /**
   * Accumulated delta range's state. It indicates whether ADR is reset or there
   * is a cycle slip (indicating loss of lock).
   *
   * This is a mandatory value.
   */
   ...
   * 'carrier phase' is given by the equation: accumulated delta range = -k *
   * carrier phase    (where k is a constant)
   *
   * This value must be populated if 'accumulated delta range state' !=
   * LOC_GPS_ADR_STATE_UNKNOWN. However, it is expected that the data is only
   * accurate when: 'accumulated delta range state' == LOC_GPS_ADR_STATE_VALID.
   */
  double accumulated_delta_range_m;
   ...
   * This value must be populated if 'accumulated delta range state' !=
   * LOC_GPS_ADR_STATE_UNKNOWN.
   */
  double accumulated_delta_range_uncertainty_m;

  /**
   * Carrier frequency at which codes and messages are modulated, it can be L1
   * or L2. If the field is not set, the carrier frequency is assumed to be L1.
   *
   * If the data is available, 'flags' must contain
   ...
   * LOC_GNSS_MULTIPATH_INDICATOR_PRESENT for those signals that are tracked,
   * and contain multipath, and LOC_GNSS_MULTIPATH_INDICATOR_NOT_PRESENT for
   * those signals that are tracked and do not contain multipath.
   */
   ...
   * If the data is available, 'flags' must contain
   * LOC_GNSS_MEASUREMENT_HAS_SNR. This is the power ratio of the "correlation
   * peak height above the observed noise floor" to "the noise RMS".
   */
  double snr_db;
} LocGnssMeasurement;
 ...
 * LocGnssSystemInfo's year_of_hw is set to 2016+, it is mandatory that these be
 * provided, on request, when the GNSS receiver is searching/tracking signals.
 *
...
typedef struct {
   ...
   * After a successful call to 'init' the HAL must begin to provide updates at
   * its own phase.
   *
   * Status:
   *    LOC_GPS_MEASUREMENT_OPERATION_SUCCESS
   *    LOC_GPS_MEASUREMENT_ERROR_ALREADY_INIT - if a callback has already been
   * registered without a corresponding call to 'close'
   *    LOC_GPS_MEASUREMENT_ERROR_GENERIC - if any other error occurred, it is
   * expected that the HAL will not generate any updates upon returning this
   * error code.
   */
   ...
   * After a call to stop, the previously registered callbacks must be
   * considered invalid by the HAL. If stop is invoked without a previous
   * 'init', this function should perform no work.
   */
  void (*close)();
...
typedef struct {
   ...
   * Parameters:
   *     config_data - a pointer to a char array which holds what usually is
   expected from file(/vendor/etc/gps.conf), i.e., a sequence of UTF8 strings
   separated by '\n'.
   *     length - total number of UTF8 characters in configuraiton data.
   *
   * IMPORTANT:
   *      GPS HAL should expect this function can be called multiple times. And
   it may be
   *      called even when GpsLocationProvider is already constructed and
   enabled. GPS HAL
   *      should maintain the existing requests for various callback regardless
   the change
