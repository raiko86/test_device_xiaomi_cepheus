...
static void convert_WGS84_to_PZ90(const LocEcef &pWGS84, LocEcef &pPZ90) {
  ...
  pPZ90.X =
      deltaX + deltaScale * (pWGS84.X + rotZ * pWGS84.Y - rotY * pWGS84.Z);
  pPZ90.Y =
      deltaY + deltaScale * (pWGS84.Y - rotZ * pWGS84.X + rotX * pWGS84.Z);
  pPZ90.Z =
      deltaZ + deltaScale * (pWGS84.Z + rotY * pWGS84.X - rotX * pWGS84.Y);
}
...
static int loc_nmea_put_checksum(char *pNmea, int maxSize) {
  ...
  int checksumLength =
      snprintf(pNmea, (maxSize - length - 1), "*%02X\r\n", checksum);
...
===========================================================================*/
static uint32_t loc_nmea_generate_GSA(
    const GpsLocationExtended &locationExtended, char *sentence, int bufSize,
    loc_nmea_sv_meta *sv_meta_p, std::vector<std::string> &nmeaArraystr) {
  ...
  for (uint8_t i = 1; mask > 0 && svUsedCount < 64; i++) {
    if (mask & 1) svUsedList[svUsedCount++] = i + svIdOffset;
    mask = mask >> 1;
  }

  if (svUsedCount == 0) return 0;
  ...
  if (locationExtended.flags & GPS_LOCATION_EXTENDED_HAS_DOP) {
    length = snprintf(pMarker, lengthRemaining, "%.1f,%.1f,%.1f,",
                      locationExtended.pdop, locationExtended.hdop,
...
static void loc_nmea_generate_GSV(const GnssSvNotification &svNotify,
  ...
  if (svCount <= 0) {
    LOC_LOGV("No SV in view for talker ID:%s, signal ID:%X", talker,
             sv_meta_p->signalId);
    return;
  }
  ...
  while (sentenceNumber <= sentenceCount) {
    ...
    length = snprintf(pMarker, lengthRemaining, "$%sGSV,%d,%d,%02d", talker,
    ...
    for (int i = 0; (svNumber <= svNotify.count) && (i < 4); svNumber++) {
      GnssSignalTypeMask signalType =
          svNotify.gnssSvs[svNumber - 1].gnssSignalTypeMask;
      if (0 == signalType) {
      ...
      if (sv_meta_p->svType == svNotify.gnssSvs[svNumber - 1].type &&
        ...
        length = snprintf(
            ...
            (int)(0.5 +
                  svNotify.gnssSvs[svNumber - 1].elevation),  // float to int
            (int)(0.5 +
                  svNotify.gnssSvs[svNumber - 1].azimuth));  // float to int
        ...
        if (svNotify.gnssSvs[svNumber - 1].cN0Dbhz > 0) {
          length = snprintf(
              ...
              (int)(0.5 +
                    svNotify.gnssSvs[svNumber - 1].cN0Dbhz));  // float to int
...
static void loc_nmea_generate_DTM(const LocLla &ref_lla,
                                  const LocLla &local_lla, char *talker,
                                  char *sentence, int bufSize) {
  char *pMarker = sentence;
  int lengthRemaining = bufSize;
  int length = 0;
  ...
  length =
      snprintf(pMarker, lengthRemaining, "$%sDTM,%s,,", talker, local_datum);
  if (length < 0 || length >= lengthRemaining) {
    LOC_LOGE("NMEA Error in string formatting");
    return;
  }
  ...
  length =
      snprintf(pMarker, lengthRemaining, "%02d%09.6lf,%c,%03d%09.6lf,%c,%.3lf,",
...
static bool get_utctime_with_leapsecond_transition(
    ...
    const LocationSystemInfo &systemInfo, LocGpsUtcTime &utcPosTimestamp) {
  ...
  // If so, per NMEA spec, we need to display the extra second in format of
  // 23:59:60 with year/month/date not getting advanced.
  if ((locationExtended.flags & GPS_LOCATION_EXTENDED_HAS_GPS_TIME) &&
    ...
    uint64_t gpsTimeLsChange =
        gpsTimestampLsChange.systemWeek * MSEC_IN_ONE_WEEK +
        gpsTimestampLsChange.systemMsec;
    uint64_t gpsTimePosReport =
        locationExtended.gpsTime.gpsWeek * MSEC_IN_ONE_WEEK +
    ...
    if (leapSecondChangeInfo.leapSecondsAfterChange >
      ...
      if ((gpsTimePosReport >= gpsTimeLsChange) &&
        ...
        // we substract 1000 milli-seconds from UTC timestmap in order to
        // calculate the proper year, month and date during leap second
        // transtion. Let us give an example, assuming leap second transition is
        // scheduled on 2019, Dec 31st mid night. When leap second transition is
        // happening, instead of outputting the time as 2020, Jan, 1st, 00 hour,
        // 00 min, and 00 sec. The time need to be displayed as 2019, Dec, 31st,
        // 23 hour, 59 min and 60 sec.
...
===========================================================================*/
static void loc_nmea_get_fix_quality(
    const UlpLocation &location, const GpsLocationExtended &locationExtended,
    bool custom_gga_fix_quality, char ggaGpsQuality[3], char &rmcModeIndicator,
    char &vtgModeIndicator, char gnsModeIndicator[7]) {
  ...
  do {
    ...
    if (locationExtended.flags & GPS_LOCATION_EXTENDED_HAS_NAV_SOLUTION_MASK) {
      if (LOC_NAV_MASK_PPP_CORRECTION & locationExtended.navSolutionMask) {
        ...
        break;
      } else if (LOC_NAV_MASK_RTK_CORRECTION &
                 locationExtended.navSolutionMask) {
        ...
        break;
      } else if (LOC_NAV_MASK_DGNSS_CORRECTION &
                 locationExtended.navSolutionMask) {
        ggaGpsQuality[0] = '2';  // 2 means DGPS fix
        ...
        break;
      } else if (LOC_NAV_MASK_SBAS_CORRECTION_IONO &
                 locationExtended.navSolutionMask) {
        ggaGpsQuality[0] = '2';  // 2 means DGPS fix
        rmcModeIndicator = 'D';  // D means differential
        vtgModeIndicator = 'D';  // D means differential
    ...
    if (true == custom_gga_fix_quality) {
      ...
      if (locationExtended.flags & GPS_LOCATION_EXTENDED_HAS_POS_TECH_MASK) {
        if (LOC_POS_TECH_MASK_SENSORS & locationExtended.tech_mask) {
          ...
          if ((LOC_NAV_MASK_RTK_FIXED_CORRECTION &
               locationExtended.navSolutionMask) ||
              (LOC_NAV_MASK_RTK_CORRECTION &
               locationExtended.navSolutionMask) ||
              (LOC_NAV_MASK_DGNSS_CORRECTION &
               locationExtended.navSolutionMask) ||
  ...
  if (generate_nmea) {
    ...
    if (location.gpsLocation.flags & LOC_GPS_LOCATION_HAS_BEARING) {
      ...
      if (locationExtended.flags & GPS_LOCATION_EXTENDED_HAS_MAG_DEV) {
        float magTrack =
            location.gpsLocation.bearing - locationExtended.magneticDeviation;
        if (magTrack < 0.0)
          magTrack += 360.0;
    ...
    // ------$--DTM-------
    // -------------------
    loc_nmea_generate_DTM(ref_lla, local_lla, talker, sentence_DTM,
                          sizeof(sentence_DTM));
    ...
    if (validFix) {
      length =
          snprintf(pMarker, lengthRemaining, "$%sRMC,%02d%02d%02d.%02d,A,",
                   talker, utcHours, utcMinutes, utcSeconds, utcMSeconds / 10);
    } else {
      length =
          snprintf(pMarker, lengthRemaining, "$%sRMC,%02d%02d%02d.%02d,V,",
                   talker, utcHours, utcMinutes, utcSeconds, utcMSeconds / 10);
    }
    ...
    if (location.gpsLocation.flags & LOC_GPS_LOCATION_HAS_LAT_LONG) {
      ...
      length =
          snprintf(pMarker, lengthRemaining, "%02d%09.6lf,%c,%03d%09.6lf,%c,",
    ...
    length = snprintf(pMarker, lengthRemaining, "%2.2d%2.2d%2.2d,", utcDay,
                      utcMonth, utcYear);
    ...
    length =
        snprintf(pMarker, lengthRemaining, "$%sGNS,%02d%02d%02d.%02d,", talker,
    ...
    length =
        snprintf(pMarker, lengthRemaining, "$%sGGA,%02d%02d%02d.%02d,", talker,
                 utcHours, utcMinutes, utcSeconds, utcMSeconds / 10);
    ...
    if (locationExtended.flags & GPS_LOCATION_EXTENDED_HAS_DOP) {
      ...
      length = snprintf(pMarker, lengthRemaining, "%s,%02d,,", ggaGpsQuality,
                        svUsedCount);
    }
...
void loc_nmea_generate_sv(const GnssSvNotification &svNotify,
  ...
  for (uint32_t svOffset = 0; svOffset < svNotify.count; svOffset++) {
    if (GNSS_SV_TYPE_GPS == svNotify.gnssSvs[svOffset].type) {
      ...
      } else if (GNSS_SIGNAL_GPS_L2 ==
                 svNotify.gnssSvs[svOffset].gnssSignalTypeMask) {
      ...
      if (GNSS_SIGNAL_GALILEO_E5A ==
          svNotify.gnssSvs[svOffset].gnssSignalTypeMask) {
        sv_cache_info.gal_e5_count++;
      } else if (GNSS_SIGNAL_GALILEO_E5B ==
                 svNotify.gnssSvs[svOffset].gnssSignalTypeMask) {
      ...
      if ((GNSS_SIGNAL_BEIDOU_B2AI ==
           svNotify.gnssSvs[svOffset].gnssSignalTypeMask) ||
          (GNSS_SIGNAL_BEIDOU_B2AQ ==
           svNotify.gnssSvs[svOffset].gnssSignalTypeMask)) {
        sv_cache_info.bds_b2_count++;
      } else if (GNSS_SIGNAL_BEIDOU_B1C ==
                 svNotify.gnssSvs[svOffset].gnssSignalTypeMask) {
  ...
  loc_nmea_generate_GSV(
      svNotify, sentence, sizeof(sentence),
      loc_nmea_sv_meta_init(sv_meta, sv_cache_info, GNSS_SV_TYPE_GPS,
