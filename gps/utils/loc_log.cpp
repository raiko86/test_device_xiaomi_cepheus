...
// Target names
DECLARE_TBL(target_name) = {NAME_VAL(GNSS_NONE), NAME_VAL(GNSS_MSM),
                            NAME_VAL(GNSS_GSS),  NAME_VAL(GNSS_MDM),
                            NAME_VAL(GNSS_AUTO), NAME_VAL(GNSS_UNKNOWN)};
...
char *loc_get_time(char *time_string, size_t buf_size) {
  ...
  snprintf(time_string, buf_size, "%s.%03d", hms_string,
