...
// The optional *goneVals*, if not null, will be populated with removed
// elements.
template <typename T>
inline static void trimSet(unordered_set<T>& fromSet,
                           const unordered_set<T>& rVals,
...
// the return set is the interset extracted out from the two input sets, *s1*
// and *s2*. *s1* and *s2* will be left with the intersect removed from them.
template <typename T>
static unordered_set<T> removeAndReturnInterset(unordered_set<T>& s1,
                                                unordered_set<T>& s2) {
...
class LocUnorderedSetMap {
  ...
  // Trim the VALs pointed to by *iter*, with everything that also exist in
  // *rVals*. If the set becomes empty, remove the map entry. *goneVals*, if not
  // null, records the trimmed VALs.
  bool trimOrRemove(
      typename unordered_map<KEY, unordered_set<VAL>>::iterator iter,
 ...
 public:
  ...
  // This looks into all the entries keyed by *keys*. Remove any VALs from the
  // entries that also exist in *rVals*. If the entry is left with an empty set,
  // the entry will be removed. The optional parameters *goneKeys* and
  // *goneVals* will record the KEYs (or entries) and the collapsed VALs removed
  // from the map, respectively.
  inline void trimOrRemove(unordered_set<KEY>&& keys,
                           const unordered_set<VAL>& rVals,
                           unordered_set<KEY>* goneKeys,
  ...
  inline void trimOrRemove(unordered_set<KEY>& keys,
    ...
    for (auto key : keys) {
      ...
      if (iter != mMap.end() && trimOrRemove(iter, rVals, goneVals) &&
          nullptr != goneKeys) {
  ...
  // This adds to each of entries in the map keyed by *keys* with the VALs in
  // the *enwVals*. If there new entries added (new key in *keys*), *newKeys*,
  // if not null, would be populated with those keys.
  inline void add(const unordered_set<KEY>& keys,
                  const unordered_set<VAL>&& newVals,
  ...
  // This puts *newVals* into the map keyed by *key*, and returns the VALs that
  // are in effect removed from the keyed VAL set in the map entry. This call
  // would also remove those same VALs from *newVals*.
  inline unordered_set<VAL> update(const KEY& key,
                                   unordered_set<VAL>& newVals) {
    unordered_set<VAL> goneVals = {};
    if (newVals.empty()) {
      mMap.erase(key);
    } else {
